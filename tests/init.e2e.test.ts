...
test("smithers init writes the expected workflow-pack layout and it typechecks", () => {
  ...
  expect(repo.exists(".smithers/workflows/ticket.tsx")).toBe(true);
  expect(repo.exists(".smithers/workflows/tickets.tsx")).toBe(true);
...
test("seeded workflows reuse the shared review substrate", () => {
  ...
  const ticketSource = repo.read(".smithers/workflows/ticket.tsx");
  const coverageSource = repo.read(".smithers/workflows/improve-test-coverage.tsx");

  expect(implementSource).toContain('../components/ValidationLoop');
  expect(ticketSource).toContain('../components/ValidationLoop');
  expect(coverageSource).toContain('../components/ValidationLoop');

  for (const workflowName of ["implement", "ticket", "improve-test-coverage"]) {
    ...
    expect(JSON.stringify(graph.json)).toContain(`${workflowName}:review:0`);
  }
});
