...
export default smithers((ctx) => {
  const collected = ctx.outputMaybe("collectedLogs", { nodeId: "collect" });
  ...
  return (
    <Workflow name="log-digest">
      <Sequence>
        ...
        <Task id="summarize" output={outputs.digest} agent={summarizer}>
          <SummarizePrompt
            errorLines={collected?.errorLines ?? []}
            warningLines={collected?.warningLines ?? []}
            rawTail={collected?.rawTail ?? ""}
            source={collected?.source ?? "unknown"}
            lineCount={collected?.lineCount ?? 0}
          />
        </Task>
      </Sequence>
    </Workflow>
