...
describe("resolveOverlayEntry", () => {
  test("resolves entry path relative to overlay dir", () => {
    const result = resolveOverlayEntry(
      "/project/src/workflow.ts",
      "/project",
      "/project/.smithers/hmr/gen-1",
    );
...
describe("buildOverlay", () => {
  ...
  test("creates generation directory with files", async () => {
    ...
    expect(readFileSync(join(genDir, "workflow.ts"), "utf8")).toBe(
      "export default {}",
    );
  });
