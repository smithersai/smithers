...
Wiki completion requires successful publication to `POST /api/gateways/{gatewayId}/wiki-pages`, authenticated with the gateway's repository-scoped bearer. The endpoint rechecks repository authority and creates revision-scoped pages idempotently. Publication failure is a failed run, never a completed but invisible Wiki. The Wiki explains the repository with verified, revision-pinned citations; categorical `verified` confidence replaces numeric confidence.
