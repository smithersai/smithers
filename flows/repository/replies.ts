...
export const replyLayers = Layer.mergeAll(Interpreter.layer(PublishReply), Publish.toLayer(({ input, result }) => Effect.gen(function*() {
  ...
  if (reply === undefined || reply.state !== "drafted" || input.configuration.replies !== "automatic" || input.event.trial === true) return drafted
  return yield* post(input, drafted, reply)
