...
export const prepareStackBase = (base: StackBase, executionId: string) => Effect.gen(function*() {
  ...
  if (!native.importSource) return yield* refused("This workspace's native helper cannot import the base; upgrade the workspace")
  ...
  if (target === undefined || target.kind !== "resolved") return yield* refused("The base was not imported as a resolved commit")
