...
const configured = (options: Options) => {
  ...
  if (options.landing !== undefined && options.planning === undefined) {
    throw new Error("Vibe requires the prompt route's project configuration as well as a landing binding")
  }
...
export const layer = (platform: NativeControl.Platform, options: Options, suppliedSeats?: SeatResolver.Service) => {
  ...
  return Layer.suspend(() => Layer.unwrap(Effect.gen(function*() {
    ...
    const catalog = Layer.unwrap(Executable.catalog({ delegates: [RunPlan, atomDelegate, checkDelegate, ...(options.planning === undefined ? [] : [RunRequest, wikiCheckDelegate]), ...(options.landing === undefined ? [] : [RunVibe])] }).pipe(
    ...
    const modules = registration.pipe(Layer.provideMerge(catalog), Layer.tap(context => Effect.gen(function*() {
      ...
      for (const [name, delegate] of [["coding", RunPlan._tag], ["coding/implementation", atomDelegate._tag],
        ...
        ...(options.landing === undefined ? [] : [["coding/vibe", RunVibe._tag]])]) {
    ...
    return Layer.effect(Serve.GatewayHost)(Effect.map(Serve.GatewayHost, gateway => ({
      launch: (health, bind, root) => gateway.launch({ ...health, gatewayId: options.gatewayId,
        capabilities: [...new Set([...(health.capabilities ?? []), "coding-plan/v1", ...(options.planning === undefined ? [] : ["coding-request/v1"]),
          ...(options.landing === undefined ? [] : ["coding-vibe/v1"])])] }, bind, root)
