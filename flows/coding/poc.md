...
The embedded run card decodes that completed native child through the same recorded ancestry and historical cursor as the plan view. It shows the latest retained prototype, or an older prototype explicitly selected in the recursive trace, and preserves complete before/after text including creation and removal. It displays those values as escaped text; it never executes the retained HTML or proposed source. Source identity must match the child's recorded input. Missing, invalid, foreign, or ambiguous native evidence does not produce a prototype claim.

While the enclosing coding run is live, “Give prototype feedback” opens the existing `runs.steer` form with the originating card and unchanged backend run ID. The form draft and actor attribution use existing state and dispatch. Its receipt means steering was queued, not that a second plan changed. Root request messages are consumed by the coordinator at its recorded safe boundaries. Only a newly completed preparation child can update the plan projection. The workflow does not pause for UI feedback automatically. Historical cursor and run selection remain the existing card state; there is no prototype database, new command, or new public API. Pure recipe value schemas live in private `poc-schema.ts`, with the prior exports retained by `poc.ts`.
