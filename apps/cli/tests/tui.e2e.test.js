...
import { launchTUI, BUN, TUI_ENTRY } from "../../../packages/smithers/tests/tui-helpers.js";
...
describe("TUI E2E", () => {
    ...
    beforeAll(async () => {
        const workflowPath = resolve(import.meta.dir, "../../../examples/fan-out-fan-in.jsx");
        testDir = await mkdtemp(join(tmpdir(), "smithers-tui-e2e-"));
        // Run a background workflow to ensure the database has something
        const proc = Bun.spawnSync([BUN, "run", TUI_ENTRY, "up", workflowPath, "-d", "--log-dir", testDir], {
            cwd: testDir,
            env: { ...process.env },
        });
        if (proc.exitCode !== 0) {
