...
function createWorkflowDir() {
  ...
  return {
    dir,
    workflowPath(name: string, exists = true) {
...
async function insertDueTimerRun(
  adapter: SmithersDb,
  runId: string,
  workflowPath: string,
  firesAtMs: number,
...
async function insertWaitingApprovalRun(
  adapter: SmithersDb,
  runId: string,
  workflowPath: string,
) {
...
async function listEvents(adapter: SmithersDb, runId: string) {
  return (await adapter.listEvents(runId, -1, 200)) as PersistedEvent[];
}
...
describe("supervisor e2e", () => {
        ...
        expect(await eventPayloads(adapter, runId, "RunAutoResumed")).toEqual([
          {
            ...
            runId,
            lastHeartbeatAtMs:
              originalHeartbeats[runId as keyof typeof originalHeartbeats],
            staleDurationMs:
              now - originalHeartbeats[runId as keyof typeof originalHeartbeats],
            timestampMs: now,
          },
        ]);
  ...
  test("supervisor handles mixed run states correctly", async () => {
    ...
    try {
      ...
      await insertDueTimerRun(
        adapter,
        "run-timer-due",
        workflows.workflowPath("run-timer-due"),
        now - 10,
      );
      await insertWaitingApprovalRun(
        adapter,
        "run-waiting-approval",
        workflows.workflowPath("run-waiting-approval"),
      );
      ...
      expect(
        await eventPayloads(adapter, "run-waiting-approval", "RunAutoResumed"),
      ).toEqual([]);
    } finally {
      sqlite.close();
      workflows.cleanup();
  ...
  test("consecutive polls dont double-resume", async () => {
    ...
    try {
      ...
      expect(await eventPayloads(adapter, "run-idempotent", "RunAutoResumed")).toHaveLength(
        1,
      );

      const supervisorEvents = await eventPayloads(
        adapter,
        SUPERVISOR_EVENT_RUN_ID,
  ...
  test("supervisor emits accurate summary metrics", async () => {
    ...
    try {
      ...
      expect(
        await eventPayloads(adapter, "run-missing-a", "RunAutoResumeSkipped"),
      ).toEqual([
        {
          type: "RunAutoResumeSkipped",
          runId: "run-missing-a",
