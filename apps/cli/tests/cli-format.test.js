...
import { colorizeEventText, formatAge, formatElapsedCompact, formatTimestamp, formatRelativeOffset, formatEventLine, } from "../src/format.js";
describe("formatAge", () => {
...
describe("formatEventLine", () => {
    ...
    test("formats NodeStarted", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "NodeStarted", payloadJson: JSON.stringify({ nodeId: "task-1", attempt: 2, iteration: 1 }) }, base);
        expect(line).toContain("→ task-1");
        expect(line).toContain("attempt 2");
        expect(line).toContain("iteration 1");
    });
    test("formats NodeFinished", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "NodeFinished", payloadJson: JSON.stringify({ nodeId: "task-1", attempt: 1 }) }, base);
        expect(line).toContain("✓ task-1");
    });
    test("formats NodeFailed with error", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "NodeFailed", payloadJson: JSON.stringify({ nodeId: "task-1", error: "timeout" }) }, base);
        expect(line).toContain("✗ task-1");
        expect(line).toContain("timeout");
    });
    test("formats NodeRetrying", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "NodeRetrying", payloadJson: JSON.stringify({ nodeId: "task-1", attempt: 3 }) }, base);
        expect(line).toContain("↻ task-1");
    ...
    test("formats RunFailed", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "RunFailed", payloadJson: JSON.stringify({ error: "boom" }) }, base);
    ...
    test("formats ApprovalRequested", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "ApprovalRequested", payloadJson: JSON.stringify({ nodeId: "gate-1" }) }, base);
        expect(line).toContain("⏸ Approval requested: gate-1");
    });
    test("formats ApprovalGranted", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "ApprovalGranted", payloadJson: JSON.stringify({ nodeId: "gate-1" }) }, base);
        expect(line).toContain("✓ Approved: gate-1");
    });
    test("formats ApprovalAutoApproved", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "ApprovalAutoApproved", payloadJson: JSON.stringify({ nodeId: "gate-1" }) }, base);
        expect(line).toContain("✓ Auto-approved: gate-1");
    });
    test("formats ApprovalDenied", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "ApprovalDenied", payloadJson: JSON.stringify({ nodeId: "gate-1" }) }, base);
        expect(line).toContain("✗ Denied: gate-1");
    });
    test("formats RunHijacked conversation mode", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "RunHijacked", payloadJson: JSON.stringify({ mode: "conversation", engine: "claude" }) }, base);
        expect(line).toContain("⇢ Hijacked claude conversation");
    });
    test("formats RunHijacked session mode", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "RunHijacked", payloadJson: JSON.stringify({ mode: "session", engine: "pi", resume: "abc" }) }, base);
    ...
    test("formats AgentEvent", () => {
        const line = formatEventLine({ timestampMs: 2000, type: "AgentEvent", payloadJson: JSON.stringify({ engine: "codex", event: { type: "tool_call" } }) }, base);
