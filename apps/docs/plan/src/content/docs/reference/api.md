...
import { Plan, PlanDiff } from "@smthrs/plan"
...
const program = Effect.gen(function*() {
  ...
  return PlanDiff.diff(plan, plan)
}).pipe(Effect.provide(NodeCrypto.layer))
```

Compiling needs Effect's `Crypto` service and nothing else. The package depends on [`@smthrs/crypto`](https://crypto.smithers.sh/reference/api/), [`@smthrs/canonical`](https://canonical.smithers.sh/reference/api/), [`@smthrs/keys`](https://keys.smithers.sh/reference/api/), and `effect`, performs no I/O, and is browser-safe. Persisting a compiled plan is [`@smthrs/plan-store`](https://plan-store.smithers.sh/reference/api/), which is where the database dependency lives.
...
Whether a value is a compiler-owned immutable snapshot that already passed `verify`. Such values take verify's trusted fast path and need no second schema validation, which is what lets [`@smthrs/plan-store`](https://plan-store.smithers.sh/reference/api/)'s `PlanStore.append` skip re-decoding them.
...
[src/Placement.ts](https://github.com/smithersai/smithers/blob/main/packages/smithers/flows/plan/src/Placement.ts)

The one placement model. [`@smthrs/core`](https://core.smithers.sh/reference/api/) re-exports it as `Placement`, [`@smthrs/flow`](https://flow.smithers.sh/reference/api/) annotates with it as `Flow.PlacementDirective`, and `@smthrs/registry` lowers a descriptor's frontmatter literal into it. A directive is serializable detail identifying a host profile; it never carries a host implementation, credentials, or any other runtime handle.

The four tags read `flows/core/Placement/...` because they are hashed into `KeyMaterial.placement`, and therefore into the step key of every node that declares a placement. Retagging them would re-key recorded work and miss its cached results, so the tag is treated as a wire value rather than as a location.

### Placement.Placement and Placement.Options
...
### Placement.local, client, sandbox and remote
...
The local process host, the viewer's browser, an isolated sandbox, and a remote control plane. `sandbox` and `remote` copy the options they are given, so a later mutation of the caller's object cannot change a recorded directive.

### Placement.Annotation

```ts
const Annotation: Context.Key<Placement, Placement>
```
...
The one cache-policy model. [`@smthrs/flow`](https://flow.smithers.sh/reference/api/) publishes it as `CacheEnvironment.CachePolicy` and [`@smthrs/engine-store`](https://engine-store.smithers.sh/reference/api/) reads it off a dispatched action; [`@smthrs/patterns`](https://smithers-patterns.smithers.sh/reference/api/) declares it over a flow as `WithCache.Policy`. It was two declarations in those two packages, joined by nothing but a re-typed identifier string, so a field added on one side was invisible to the other.
...
The one bounded-repetition ceiling vocabulary. [`@smthrs/flow`](https://flow.smithers.sh/reference/api/)'s `Poll.make` takes it as `onTimeout`, for the attempt a poll gives up on, and [`@smthrs/patterns`](https://smithers-patterns.smithers.sh/reference/api/)' `Loop` takes it as `onMaxReached`, for the iteration a loop gives up on.
