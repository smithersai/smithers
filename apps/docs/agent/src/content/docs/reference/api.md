...
| `capacity`           | `{ park: boolean; maxParkMillis?: number }`      | Capacity handling. Parking defaults to true, with no wait ceiling; unknown resets are re-probed after 15 minutes.                                                                    |
