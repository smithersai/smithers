...
It declares one classifier, `completion/claim`, over four facts the harness
...
reads. The thresholds are strict because the vendor reports 76% agreement with
frontier-model labels on its own evaluations: a false demand costs a run a
frame, while a false pass costs nothing here, because the five deterministic
brakes still ran and the truth bar still judges what they let through.
