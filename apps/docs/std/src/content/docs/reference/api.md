...
| `Classify`     | `MAX_STATES`, `MAX_STATE_BYTES`, `CONCURRENCY`, `State`, `Questions`, `States`, `Verdict`, `BatchResult`, `ask`, `askAll`, `answerContract`, `curated`; `run` fails with `ClassifierError` |
...
`Classifier.make` declaration from [`@smthrs/model`](https://model.smithers.sh/reference/api/) with a state
...
| `checkVerdict`     | `Classifier` `check/verdict`     | `{ task, command, exitCode, output }`: `rightReason` boolean, `invalidProbe` boolean.                                      |
| `editRisk`         | `Classifier` `edit/risk`         | `{ task, path, hunk }`: `risk` score, `reversible` boolean.                                                                |
| `probeAttribution` | `Classifier` `probe/attribution` | `{ command, exitCode, output }`: `attribution` choice, `executed` boolean. Asked by `Probe.classify`, not bound as a flow. |
| `all`              | `ReadonlyArray<AnyClassifier>`   | The first three, in catalog order; what `StandardFlows.classify` binds when a host names no classifiers.                   |
...
`{ states }` and whose description ends with `Classify.answerContract`, the
answer ids and shapes derived from the questions. See
...
The attribution is Jev's: `classify` asks the `probe/attribution` classifier
through the `Evaluator` service of [`@smthrs/model`](https://model.smithers.sh/reference/api/), so a host
needs `AI_GATEWAY_API_KEY` and a bound evaluator. Two things are still decided
without it, because an exit code is a fact rather than prose: a zero exit is
never classified, and 126 and 127 are the shell's own refusal to start the
command. An answer below `CONFIDENCE_FLOOR` is the tree's failure, which leaves
a genuine reproduction intact. There is no third path: when the judge does not
answer, `unjudged` turns its failure into the caller's typed failure rather
than a guess.
