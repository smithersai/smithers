...
| `layerFromEnvironment(environment)`                                                             | layer                     | The evaluator a host binds from its own environment: `layerVercelGateway` when `AI_GATEWAY_API_KEY` is set, `layerUnavailable()` when it is not. Every host that runs an agent loop binds this, because the harness's completion brake never falls back. Needs the kernel `HttpClient`.                                                                                                                        |
