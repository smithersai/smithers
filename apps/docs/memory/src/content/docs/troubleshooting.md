...
An empty snapshot is a successful read that found no rows, or a `maxBytes` too small for the fence and the first row's label. Primer reads use a bounded newest-first candidate window; facts can occupy candidate slots but only notes render as primers. See [opening memory](/guides/agent-opening-context/) for truncation order.
