...
`abandon` after the frame are undone too; it runs `jj op restore`, which also
resets other workspaces' working-copy commits to that operation. A frame whose
anchor recorded no operation refuses it with `irreversible`.
