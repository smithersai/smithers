...
`captureSnapshot({ sdk, machine, name, stopTimeoutMs? })` stops a machine, captures its root disk as the named snapshot, and removes the machine whether or not the capture succeeded. `hasSnapshot(sdk, name)` answers whether one exists (a `SnapshotNotFound` answer is `false`; any other failure is `unavailable`). `pruneSnapshots(sdk, prefix, keep)` removes the snapshots named with `prefix` beyond the `keep` newest and returns their names. A machine booted from a snapshot does not depend on it afterwards, so pruning never disturbs a running machine.
