...
## Signup onboarding (Will, 2026-09-20)

A signed-out cloud visitor (and any session whose signup stage is short of `done`) sees the signup in the transcript, never a page of its own: an animated title-sized "Automate your codebase today" over the sign-in doors (GitHub = `auth.sign-in`; Google and company email = `signup.google` / `signup.email`, which refuse with a typed toast on a host without those doors), a 6-digit email code, "Finish creating your account" (full name + `smithers.sh/<account>`, the GitHub login prefilled), then a poll one question at a time with lettered choices, none required (company size, role, how they heard, know what to automate, preferred models, repository to connect, anything else), then "Your workspace is ready!" with one giant Start Automating button and the tutorial video slot. Every stage and answer lives on `session.signup` (`state/Signup.ts`) so a reload resumes it; earlier stages collapse to receipts. Start Automating sets the stage to `done`, after which every visit shows the first-run below. A session saved before this field existed skips the signup when signed in. Reference: `cards/SignupCards.tsx`, `state/controller/signup.ts`, `flows/entries/signup.ts`, `e2e/playwright/signup.spec.ts`.

## First-run experience (Will, 2026-09-15, after the signup since 2026-09-20)

Open the app directly. New sessions show a setup checklist (Connect GitHub → Add a repository → Set up a job) whose steps check off from live state and whose buttons are those steps' flows; it hides itself once every step is complete. Once the third step is done, that step IS the repository's five job buttons, and the row outlives the checklist: with the recommended actions dismissed, every conversation still reaches every job in one press, each button reading the state its card reads (amended 2026-09-18). Below it, registry-backed recommended actions, grouped by namespace, show until dismissal: dispatching a job opens that job's card and leaves every job one button away, and a configured job's button reads the state its card reads. Signed-out visitors start with the practice repository behind the signup. First-sight hints use HelpBubble, persist dismissal, and show one at a time in DOM order. Command/Control-K opens Chat; touch hints omit key names. There is no scripted lesson sequence.
...
See the [permanent MINIMAL TEXT rule](../../AGENTS.md).
