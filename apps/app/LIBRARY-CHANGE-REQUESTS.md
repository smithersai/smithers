# Library change requests from the apps/app flow port
...
- **Why it matters here**: every flow in this app claims the vocabulary that
  ...
  `apps/app/src/mainview/chain/AppEngine.test.ts` pins this as a passing test.
...
- **Workaround taken**: `apps/app/src/mainview/FlowStamp.ts` stamps `data-flow`
...
- **Landed**: `submitProps` / `stopProps` on `ChatComposerProps` and
  ...
  correct one. `apps/app/src/mainview/FlowStamp.ts` dropped its ref callback
...
- **Workaround taken**: `apps/app/src/mainview/FocusRing.ts` restores the
...
- **Landed**: `escapeTabOrder` on `MarkdownEditorProps`, default true. A
  ...
  textarea. `apps/app/src/mainview/FocusRing.ts` was deleted on 2026-09-09; the
...
- **Workaround taken**: `apps/app/src/mainview/RichMarkdown.tsx` splits table
...
- **Landed**: exactly that, in `renderBlocks` and in `splitBlockSources` so a
  ...
  `apps/app/src/mainview/RichMarkdown.tsx` was deleted on 2026-09-09: the
