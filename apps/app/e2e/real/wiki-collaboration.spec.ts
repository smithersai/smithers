...
test(
  ...
  async ({ page, context }) => {
    ...
    await expect(pane.locator('[data-slot="file-tree-file"]').filter({ hasText: first.title })).toBeVisible()
    await expect(pane.locator('[data-slot="file-tree-file"]').filter({ hasText: second.title })).toBeVisible()
