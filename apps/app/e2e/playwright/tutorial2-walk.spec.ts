...
test("the whole tutorial walks every beat, keyboard first, through asynchronous live API doubles", async ({ page, baseURL }) => {
  ...
  // Back and its button revisit the completed beat; its original pill/key can continue.
  for (const pointer of [false, true]) {
    if (pointer) await page.locator('.guide-navigation [aria-keyshortcuts="b"]').click()
    else await page.keyboard.press("b")
    await atStage(page, 2)
    const read = page.locator('.guide-actions [aria-keyshortcuts="r"]')
    await expect(read).toHaveAttribute("data-done", "true")
    await expect(card(page, "issue")).toBeVisible()
    if (pointer) await read.click()
  ...
  await expect(page.locator('.guide-actions [aria-keyshortcuts="g"]')).toBeEnabled()
  await expect(goalMark(page, "commits")).toHaveAttribute("data-done", "true")
  await page.keyboard.press("g")
  await atStage(page, 7)
  expect(host.live.filter(call => call.operation === "implement")).toHaveLength(1)
  ...
  await page.keyboard.press("b")
  await atStage(page, 7)
  await expect(card(page, "diff")).toBeVisible()
  await expect(card(page, "file")).toHaveCount(0)
...
test.describe("beat 9: four commit selections are sent to the live Change operation", () => {
  ...
  test("live implementation commits can be selected individually and Back restores the previous pick", async ({ page, baseURL }) => {
    ...
    await expect(followup(page, 9).locator(".guide-step-done")).toBeVisible()
    await page.reload()
    await rebooted(page)
    await atStage(page, 9)
    ...
    await expect(card(page, "change")).toContainText("1 commits selected for review")
    expect(host.live.filter(call => call.operation === "change")).toHaveLength(1)
    await page.keyboard.press("b")
    await atStage(page, 9)
    await expect(card(page, "commit-pick")).toBeVisible()
    await page.keyboard.press("b")
...
test.describe("escape hatches", () => {
  test("Skip practice (Q) on beat 3 lands on the bridge and marks the goal Skipped", async ({ page, baseURL }) => {
    ...
    await expectBeat(page, 3)
    await expect(card(page, "issue")).toBeVisible()
    for (const step of [4, 5, 6, 7, 8, 9]) await expect(line(page, step)).toHaveCount(0)
    await page.locator('.guide-navigation [aria-keyshortcuts="b"]').click()
    await atStage(page, 2)
    // A second skip after rewinding must remember 2, despite the later completion receipts.
    await page.keyboard.press("q")
    await atStage(page, 10)
    await page.locator('.guide-navigation [aria-keyshortcuts="b"]').click()
    await atStage(page, 2)
  ...
  test("Skip at beat 1 and Back never visit undone practice beats", async ({ page, baseURL }) => {
    ...
    for (const pointer of [false, true]) {
      ...
      await expect(shell(page)).toHaveAttribute("data-stage", "1")
      await expect(page.locator('.guide-navigation [aria-keyshortcuts="b"]')).toHaveCount(0)
      for (const step of [4, 5, 6, 7, 8, 9]) await expect(line(page, step)).toHaveCount(0)
    }
    await doBeat(page, 1, "i")
    await atStage(page, 2)
