    ...
    const services: AppServices = {
      fetchImpl: async (input) => {
        ...
        if (path === "/api/cloud/api/repos/will/flows/workspaces/ws-1/files") {
          ...
          if (at === "apps") return json(200, { path: "apps", entries: [{ name: "ui", path: "apps/app", type: "dir", size: 0 }] })
  ...
  const sharedHarness = async () => {
    ...
    const services: AppServices = {
      fetchImpl: async (input) => {
        ...
        if (path === `${contents}/apps`) return json(200, [{ name: "ui", path: "apps/app", type: "dir", sha: "", size: 0 }])
