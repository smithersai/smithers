...
for (const search of ["", "?tutorial"]) test(`home pin adopts only a real repository${search}`, () => {
  const { host, entries } = browser(`/${search}`)
  const history = createBrowserFrameHistory(host, { keepUrl: true })
  history.replace(root)
  history.selectRepository?.("practice:smithersai/hello-server")
  expect(entries()).toEqual([`/${search}`])
  history.selectRepository?.("acme/widgets")
  expect(entries()).toEqual([`/acme/widgets${search}`])
  expect(history.current()).toEqual(root)
  history.push(card)
  history.selectRepository?.("other/repo")
  expect(host.location.pathname).toBe("/acme/widgets")
  expect(createBrowserFrameHistory(host).current()).toEqual(card)
  history.back()
  expect(history.current()).toEqual(root)
