...
export const APP_TRANSITION_SCHEMAS = {
  ...
  "identity.session.loaded": z.object({ "type": z.literal("identity.session.loaded"), provider: z.enum(["github", "local"]).optional(), "actor": z.literal("system"), "state": z.enum(["signed-out", "signed-in", "unavailable"]), "login": z.union([z.string(), z.null()]), "allowlisted": z.boolean(), "admin": z.boolean(), "scopesPlain": z.union([z.string(), z.null()]) }).strict(),
