...
const entry = { kind: "wiki" as const, repo: "will/demo", scope: "this-attempt", phase: "launching" as const, startedAt: 100 }
const run = (id: string, ordinal: number, lastSeq: number, phase: "running" | "completed" = "running"): Extract<Card, { kind: "run-trace" }> => ({
  id, ordinal, createdAt: 100, kind: "run-trace", status: "active", title: "Wiki",
  payload: { repo: entry.repo, runId: "run-1", workflow: "librarian/wiki", phase, steps: [], lastSeq, result: null,
...
test("recovery matches the attempt's repo, scope, kind and receipt without borrowing an older run", () => {
  ...
  expect(librarianReceiptFor([card], { ...entry, kind: "history" })).toBeUndefined()
