...
for (const mode of ["valid", "stale", "wrong-parent", "failed", "account-changed"] as const) test(`completion requires persisted verified receipt: ${mode}`, async () => {
  ...
  await store.dispatch({ type: "guide.changed", actor: "user", guide: { ...initialGuide(), step: 7, playthrough: mode === "stale" ? 2 : 1 } }).isPersisted.promise
  ...
  expect(store.session().guide?.completed?.includes("change.committed") ?? false).toBe(mode === "valid")
