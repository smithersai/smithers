...
test("user and agent select the same persisted payload without completing diff review after real scoped trace inspection", async () => {
  const step = GUIDE_STAGES.findIndex(stage => stage.kind === "do" && stage.completion === "diff.opened")
  if (step < 0) throw new Error("Missing diff.opened lesson")
  for (const actor of ["user", "smithers"] as const) {
    ...
    await store.dispatch({ type: "guide.changed", actor: "user", guide: { ...initialGuide(), step, completed: ["commits.made"], playthrough: 2 } }).isPersisted.promise
    ...
    expect(store.session().guide?.completed).not.toContain("diff.opened")
    ...
    expect([...store.collections.transitions.values()].filter(row => row.type === "card.updated").at(-1)?.actor).toBe(actor)
    await store.dispose?.()
  }
})
