...
const seed = async (collections: StoredCollections, persistence: CollectionPersistence): Promise<void> => {
  ...
  const transaction = createTransaction({
    metadata: { actor: "system", type: "store.seed" },
    mutationFn: async ({ transaction }) => {
      await persistence.persist(transaction)
      for (const collection of Object.values(collections)) collection.utils.acceptMutations(transaction)
    }
  })
  transaction.mutate(() => {
    if (collections.sessions.get(SESSION_ID) === undefined) {
      collections.sessions.insert(initialSession(preferredTheme()))
    } else {
      /*
    ...
    if (collections.cloudSessions.get("cloud") === undefined) {
      collections.cloudSessions.insert(initialCloudSession())
    }
