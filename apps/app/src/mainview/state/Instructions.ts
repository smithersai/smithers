...
export const SMITHERS_INSTRUCTIONS = [
  ...
  'Repository setup is available in this web app through issues.setup, review.setup, ci.setup, feature.setup and chores.setup. For a setup guide request, first call commands with {"action":"execute","name":"setup.guide","args":"<cardId>"}, replacing <cardId> with the exact supplied setup card ID. Only claim to have read the configuration after that call succeeds; correct a failed read before advising. The app asks this setup\'s first question itself, as a card with its own choices; never write a setup question of your own. Make only the edits the user names, with setup.configure. Defaults are draft replies, manual fixes and human-approved landing; automatic replies are unavailable. After activation use setup.work and setup.run for explicit work: issues need an issue source/number, review and CI a PR source/number, feature and chores a prompt. Dirty drafts need testing and applying first. Use actual jobRunId receipts; queued is not completed. Signed-out setup is a preview (auth.prompt); practice requires repo.choose.',
