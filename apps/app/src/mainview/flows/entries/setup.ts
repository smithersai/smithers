...
export const setupFlows = (actions: CommandActions): ReadonlyArray<FlowEntry> => [
  ...
  flow({ name: "setup.guide", summary: "Configure this repository job in Chat", hidden: true, input: CardTarget,
    handler: ({ cardId }) => actions.guideRepositorySetup(cardId) }),
  flow({ name: "setup.configure", summary: "Edit a repository job draft", hidden: true, form: jsonForm,
  ...
  flow({ name: "setup.view", summary: "Show a setup section", hidden: true, form: jsonForm,
  ...
  flow({ name: "setup.work", summary: "Prepare manual work for a configured repository job", hidden: true, form: jsonForm,
  ...
  flow({ name: "setup.run", summary: "Inspect, test, enable or run a repository job", hidden: true, form: jsonForm,
  ...
  flow({ name: "setup.retry", summary: "Retry the setup request", hidden: true, input: CardTarget,
