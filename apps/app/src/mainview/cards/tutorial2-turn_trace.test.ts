...
test("trace completion requires this playthrough's committed change, selected repo and actual source/call subtree", () => {
  const guide = { ...initialGuide(), step: 8, playthrough: 3, completed: ["change.committed"] }
  ...
  expect(check({ ...guide, step: 7 })).toBe(false)
...
test("structured redaction metadata never becomes recorded executable source", () => {
  ...
  expect(canCompleteTutorialTrace({ ...initialGuide(), step: 8, completed: ["change.committed"] },
