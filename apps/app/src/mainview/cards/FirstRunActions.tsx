...
export function FirstRunActionsCard({ commands, state, repo, onRunCommand: dispatchFlow, onDismiss }: {
  ...
  return <section className="first-run-actions" data-testid="first-run-actions" aria-label="Recommended actions">
    ...
    {firstRunGroups(commands, state).map(group => <section key={group.namespace} aria-label="Repository jobs">
      {group.flows.map(flow => <button type="button" key={flow.name} {...flowAction(onRunCommand, runtimeFlowName(flow.name), repo)}>{flow.summary}</button>)}
    </section>)}
  </section>
}
