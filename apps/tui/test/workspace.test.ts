...
const setup = (options: { readonly known?: boolean } = {}) => {
  ...
  const host: Host.Host = {
    ...
    run: (input) => {
      inputs.push(input)
      return { done: new Promise((resolve) => finishes.push(resolve)), cancel: () => finishes.at(-1)?.({ _tag: "cancelled" }) }
    }
  }
  let current = listed
  ...
  return { workspace, inputs, finishes, loads, records, relist: (next: ReadonlyArray<Extension.Descriptor>) => { current = next } }
}
