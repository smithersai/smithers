...
The `@next` suffix is required: release candidates publish to the `next` dist-tag, and `latest` still resolves the Smithers 0.x line. Effect is pinned to exactly `4.0.0-rc.115`. For the `smthrs` command, install `npm install --global @smthrs/cli@next`.
