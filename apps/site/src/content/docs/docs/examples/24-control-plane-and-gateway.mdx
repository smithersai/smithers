...
Control state and execution state use separate databases while the journal supplies the event stream. Both transports authenticate with a fresh session bearer credential. Exact Host and Origin checks protect the loopback listener; binding to loopback alone does not authenticate callers.
...
[`@smthrs/control`](/docs/reference/api/control/) · [`@smthrs/database`](/docs/reference/api/database/) · [`@smthrs/flow`](/docs/reference/api/flow/) · [`@smthrs/notifications`](/docs/reference/api/notifications/) · [`@smthrs/platform-node`](/docs/reference/api/platform-node/) · [`@smthrs/registry`](/docs/reference/api/registry/) · [`@smthrs/run-store`](/docs/reference/api/run-store/)
