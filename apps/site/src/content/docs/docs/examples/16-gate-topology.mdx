---
title: "Gate topology"
description: "The release gate's checks and the topology that batches them."
editUrl: "https://github.com/smithersai/smithers/edit/main/examples/src/16-gate-topology.ts"
sidebar:
  order: 16
---

{/* Generated from examples/src/16-gate-topology.ts by `node apps/site/scripts/gen-examples.mjs`; edit the example, not this page. */}

The release gate's checks and the topology that batches them.

It lives beside `16-fan-out-fan-in.ts` rather than inside it because two modules declare the same gate: that example, which names it in code, and `16-project/flows/gate/flow.ts`, which is the file a project puts on disk for discovery to find. A shared declaration that lived in either one would make the other import it back, and a module cycle is not a shape a flow file can have: discovery imports the file to run it.

## Run the test

The complete program is [`examples/src/16-gate-topology.ts`](https://github.com/smithersai/smithers/blob/main/examples/src/16-gate-topology.ts). Clone the repository and follow [CONTRIBUTING.md](https://github.com/smithersai/smithers/blob/main/CONTRIBUTING.md) to install and build its packages.
...
```ts title="examples/src/16-gate-topology.ts"
