...
Only read/write flows are exposed, with capabilities scoped to the scratch root and a host service that resolves relative paths there and rejects absolute paths, traversal and links. The host must exclusively own the tree during the run; this is not an OS boundary against concurrent host mutations. The scripted seat makes the test deterministic and requires no API key.
...
[`@smthrs/agent`](/docs/reference/api/agent/) · [`@smthrs/capability`](/docs/reference/api/capability/) · [`@smthrs/flow`](/docs/reference/api/flow/) · [`@smthrs/harness`](/docs/reference/api/harness/) · [`@smthrs/journal`](/docs/reference/api/journal/) · [`@smthrs/model`](/docs/reference/api/model/) · [`@smthrs/registry`](/docs/reference/api/registry/) · [`@smthrs/std`](/docs/reference/api/std/)
