...
3. `call_read_tool` calls a tool whose `readOnlyHint` is `true`.
4. `call_write_tool` calls a tool whose `readOnlyHint` is `false`.

Every command declares `readOnlyHint`, and each wrapper refuses the other kind. `flow_list`, `runs_list`, and `runs_show` are read-only; `flow_plan` is not, because it records the plan and its approval token.
