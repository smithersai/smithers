...
You need no provider key: both the model and the completion evaluator are scripted, so the runs are deterministic. A live host supplies a provider-backed seat and a real evaluator; the scripted completion probabilities below are only for this tutorial.
