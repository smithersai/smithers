...
Use the project from [Build your first agent flow](/docs/tutorials/first-agent-flow/), with its library install, and a supported Node.js version (`>=26.4.0`). Add the durable engine dependencies:
...
const stubJj = Layer.succeed(
  Jj.Jj,
  Jj.make({
    snapshot: () => Effect.succeed({ changeId: "tutorial-snapshot" as never, commitId: "tutorial-snapshot" as never }),
    restore: () => Effect.void,
    diff: () => Effect.succeed(""),
