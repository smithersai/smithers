...
A `GET` or `HEAD` of a former documentation address under `/rpc/` (a page the
old site published) redirects to this page instead; an RPC-shaped request there
is still the 410. Cross-origin requests may be refused earlier by the
same-origin guard.
