...
| `model` | string | no | none | The model seat the flow runs on, for example `openai:gpt-6-sol`. An empty value reads as absent, and `smthrs up` cannot launch a prompt flow that declares no seat. |
