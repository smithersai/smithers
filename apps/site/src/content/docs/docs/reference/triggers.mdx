...
The `triggers.list` flow (slash command `/triggers.list`, or ask the agent to list the triggers on a repository) renders the Dispatcher card: declared factory rules and live rows with the schedule, flow, enabled state, and fire times. `/triggers.register` prepares a plan; human approval registers it through the repository registrar. Named registrations expose **Run now** and **Pause**. See [Dispatcher](/docs/app/dispatcher/).

Live state requires a connected workspace. An unavailable gateway or registrar produces a visible failure. Signed-out visitors can inspect declared rules; writes require sign-in.
...
There is no `smthrs webhooks` verb. The Dispatcher card displays webhook channels when its workspace returns them. For the library's module contract, see [`@smthrs/triggers`](/docs/reference/api/triggers/#webhook).
