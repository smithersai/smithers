...
| `call_read_tool` | `name` and `arguments` (an object, default empty) | Executes a tool whose `readOnlyHint` is `true`. |
| `call_write_tool` | `name` and `arguments` (an object, default empty) | Executes a tool whose `readOnlyHint` is `false`. |
...
Replace `/work/project` with the absolute project path. `flow_list` is read-only, so call it through `call_read_tool`; a local listing reads the discovery snapshot and opens no project database. Every tool declares `readOnlyHint`, and each wrapper refuses the other kind, so choose the wrapper from that annotation. `flow_plan` is not read-only: it records the plan and its approval token.
