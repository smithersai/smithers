...
| --- | --- |
| `<Workflow>` and the other JSX components | A module flow at `flows/<name>/flow.ts`, which `export default`s `Flow.make(tag, { description, payload, success, error, body })`. The JSX root has no counterpart; the flow declaration is the root. |
...
| `createSmithers` and `runWorkflow` | `Flow.make(tag, { payload, success, error, body })` and `flow.execute(payload, { executionId })`, run by a host that provides its action implementations and engine. `smthrs up` executes a module flow's own body, and a markdown agent flow's prompt, directly. |
