...
import { TurnLimits, anonymousTurnKey, turnLimitResponse, ANONYMOUS_CEILING, ANONYMOUS_ALL_CEILING } from "./turnLimit"
...
export const handleLiveTutorial = (request:Request) => Effect.gen(function*(){
  ...
  if(operation){
    ...
    if(!local.allowed)return turnLimitResponse(local,{},ANONYMOUS_CEILING)
    const shared=yield* limits.spend("tutorial:all",ANONYMOUS_ALL_CEILING)
    if(!shared.allowed)return turnLimitResponse(shared,{},ANONYMOUS_ALL_CEILING)
    if(!session){
