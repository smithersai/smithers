...
import { REPOSITORY_SETUP_API, SetupHostInputSchema, SetupOperationSchema } from "@smthrs/rpc/RepositorySetup"
...
const publicResult = (record: SetupRecord) => ({ ...(record.result ?? { requestId: record.input.requestId, revision: record.input.revision, digest: record.input.digest, receipt: record.receipt }),
...
export const handleRepositorySetup = (request: Request): Effect.Effect<Response, never, Services | ExecutionContext> => Effect.gen(function* () {
  ...
  } else if (request.method === "GET" && part === "request") {
  ...
  return record.observationError ? answer(503, { message: record.observationError }) : answer(record.result ? 200 : 202, publicResult(record))
