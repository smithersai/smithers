...
**Cutover is held.** Complete `docs/shared-edge-cutover.md` before landing this
candidate. A push to main deploys the edge; the currently live stateful Worker
must retain access until the migration and drain are verified.
...
in `docs/legacy-worker-deploy.md` for rollback only.
...
- 2026-09-24 — **prepared, not deployed**: switch `src/index.ts` to `src/edge.ts`;
  ...
  rollback. The detailed historical log is in `docs/legacy-worker-deploy.md`.
