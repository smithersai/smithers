...
Activation replaces the final fence with the exact artifact the release gate rehearsed. Its digest is in `deploy.ts --dry-run`'s receipt (`artifactSHA256`). The gate's hook (`SMITHERS_EDGE_ACTIVATION_AUTHORIZE`, subcommand `authorize-edge`) must authorize it while holding the production lease and naming the verified cutover and import receipts. There is no override flag. An unrecognized or contradictory live version refuses.

`wrangler.jsonc` stays on the legacy entry until the cutover activates. The shared-edge candidate is `wrangler.edge.jsonc`, documented in `docs/shared-edge-deploy.md`. Swapping it into `wrangler.jsonc` before the live version is the cutover fence makes every deploy refuse with `DEPLOY_GUARD_EDGE_BEFORE_CUTOVER`; that happened from 5d776f34b until the legacy entry was restored (smithers#1729).
