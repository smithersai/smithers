 ...
 * returned by any GET, matching the provider.
 */
...
export interface FakeWorker { versions: Map<string, FakeVersion>; live: string; deployments: Array<{ id: string; created_on: string; versions: Array<{ version_id: string; percentage: number }> }>; subdomain: { enabled: boolean; previews_enabled: boolean }; routes: string[]; domains: string[]; schedules: string[] }
...
export class FakeCloudflare {
  ...
  constructor(sources: Record<string, string> = {}, kvBacked: string[] = []) {
    ...
    for (const worker of CLOUDFLARE_PRODUCERS) {
      ...
      this.workers.set(worker, { versions: new Map([[version.id, version]]), live: version.id, deployments: [{ id: randomUUID(), created_on: new Date(Date.now() - 86_400_000).toISOString(), versions: [{ version_id: version.id, percentage: 100 }] }],
  ...
  addRehearsal(worker: string) {
    ...
    this.workers.set(worker, { versions: new Map([[version.id, version]]), live: version.id, deployments: [{ id: randomUUID(), created_on: new Date().toISOString(), versions: [{ version_id: version.id, percentage: 100 }] }],
  ...
  setLive(worker: string, entry: string, message: string | null, extraModules: string[] = []) {
    ...
    w.versions.set(version.id, version); this.deploy(w, version.id)
    return version.id
  }
  ...
  private async handle(request: Request): Promise<Response> {
    ...
    if (rest === "/settings") return this.ok(this.settings(this.live(worker)))
    ...
    if (rest.startsWith("/versions/")) { const v = w.versions.get(rest.slice(10)); return v ? this.ok({ id: v.id, metadata: { source: "api" }, annotations: { ...v.message ? { "workers/message": v.message } : {}, "workers/triggered_by": "upload" } }) : Response.json({ success: false }, { status: 404 }) }
    if (rest === "/content/v2") {
      const v = this.live(worker), form = new FormData()
    ...
    if (rest === "" && method === "PUT") {
      ...
      const previous = this.live(worker), secrets = metadata.keep_bindings?.includes("secret_text") ? { ...previous.secrets } : {}
