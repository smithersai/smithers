...
The configured coding host uses the existing wiki flow before planning and registers `coding/WikiCheck` as an ordinary slow-check delegate. This repository's `factory/coding/project.ts` includes it as a required check alongside existing PACKAGE targets. Planning retains all operator-required checks on every Change. The check captures the exact immutable implementation source, records its evidence before scoped scratch closes, and calls the existing reviewer and assessor in the same native runtime. Unsupported or uncertain prose produces an existing coding Receipt with owner findings. Invalid citations receive one bounded semantic repair and refuse review evidence if exact validation still fails. No second runtime, database or publication pointer is created by a slow check.

The configured host binds its actual source or deployment-artifact reviewer fingerprint into approval and receipt-reuse identities. Target repositories need not vendor Smithers reviewer files. Standalone wiki execution retains its existing four-source policy. Configured host publication must be outside the source workspace and revalidates that boundary immediately before writing; the standalone command above still generates inside its own workspace. Final verified publication remains source-fenced and separate from intermediate check receipts. See [the semantic check](../../flows/coding/wiki-check.md) for the private capture data and shared Registry composition.
