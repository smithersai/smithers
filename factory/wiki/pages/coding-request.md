...
The result pairs the second prepared `plan` with the correction `outcome`. Its domain status is `validated`, `changes-requested` or `blocked`. The first implementation counts toward `maxRounds`; the default is three and the admitted range is one through eight.
