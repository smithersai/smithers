...
for item in "$backend" /opt/smithers/bin/node /opt/smithers/bin/smithers-coding-host /opt/smithers/bin/smithers-librarian-host /opt/smithers/bin/smithers-jj-export pg_dump pg_restore psql; do [ -x "$item" ] || command -v "$item" >/dev/null 2>&1 || die "packaged runtime is unavailable: $item"; done
...
(cd /opt/smithers/bin && sha256sum -c smithers-coding-host.sha256 smithers-librarian-host.sha256 >/dev/null) || die "packaged Flow host checksum failed"
...
export PATH="/opt/smithers/bin:/usr/lib/postgresql/18/bin:$PATH"
state=$(state_file)
if [ ! -f "$state" ]; then
