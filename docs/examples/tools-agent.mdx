...
Smithers agents can be given **tools** that let them interact with the filesystem and shell. This is useful for tasks like codebase analysis, log searching, or automated refactoring. The built-in tools from `smithers-orchestrator` provide `read`, `write`, `edit`, `grep`, and `bash` capabilities.
...
| `write` | Write content to a file by path. Creates the file if it does not exist. |
| `edit` | Apply targeted edits to a file using search-and-replace. Safer than full rewrites for small changes. |
