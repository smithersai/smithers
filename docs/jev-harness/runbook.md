...
Known open: a resume is not proven on the current revision. On 2026-09-19, after whole-completion-decision journaling and an at-source observer checkpoint landed, a browser run killed a server just after a settled marker call and restarted it healthy in 7.8 s, and the resumed turn then never finished: `GET /session/status` still read `busy` and `GET /permission` was empty at the 240 s bound. Every other scenario in that same run passed. An earlier run on the revision before that merge resumed and finished in 44.4 s with the marker having run exactly once. So a restart may leave a turn busy with nothing to answer. If that happens, the marker will not have run twice; stop the server and start it again, and send the prompt as a new turn.
