...
A side effect is any mutation of state **outside the sandbox**. If the tool talks to an external API, writes to a database, sends a message, or triggers a webhook, it has a side effect. Mark it.
