...
When `allowNetwork` is `false` (the default), the tool checks whether the command string (executable + arguments joined together) contains any blocked fragment. If a match is found, the command is rejected with an error.
...
Git remote operations are only checked when `git` appears in the command string. Local `git` commands (e.g., `git status`, `git diff`, `git log`) are allowed even when network access is disabled.

> **Note:** The current blocked list is intentionally minimal. Additional commands and detection patterns (e.g., remote shells, network utilities, proxy arguments) may be blocked in future versions.
