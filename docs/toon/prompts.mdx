...
**Keep expressions readable.** Ternaries and method calls work inside `{}`, but if an expression gets complex, use an inline `run:` block to prepare the data and reference its output in a downstream prompt.
