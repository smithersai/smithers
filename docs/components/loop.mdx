...
Both names render the same `<smithers:ralph>` host element. Many composite components (Supervisor, ReviewLoop, Optimizer, Debate) use `Loop` internally for their iteration logic.
