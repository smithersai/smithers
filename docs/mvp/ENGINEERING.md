...
Jev also decides the issue job's `duplicates` step, and it is the only model that decides it. Finding a duplicate is a pairwise judgment over the closed list of prior records the job already captured, so [`jev-duplicates.ts`](../../flows/repository/jev-duplicates.ts) sends one state per captured issue, the request's own title and body beside that record's, both bodies clipped so the state stays under 32 KiB, batched 64 states at a time. One ordered question scores the pair `distinct: different defects or requests`, `related: same area or symptom, different cause` or `same: the same underlying defect or request`, and the instructions carry the rule the old system prompt carried: similar components, similar wording or the same file alone do not establish a duplicate. A record is reported as a duplicate only at `same` with confidence 0.8 or above, the same top-of-range bar the intake screen uses and for the same reason, because a wrong match closes a live report against an unrelated one. The host then builds the whole `Observation` from those answers with no model prose in it: the matched records become `duplicates` with the level's own text as the reason, their URLs become the citations when the captured evidence already allows them, the summary is a count sentence, the classification is the intake screen's own `kind` (`spam` reads as `irrelevant`, an unscreened event as `unknown`), the question is empty and the reproduction is null. A job whose capture found no prior issue answers the empty observation without asking Jev anything. There is no fallback and no seat: the `repository/duplicates` agent action is deleted, and an evaluator that is unconfigured, unreachable, refused, timed out or malformed fails the step with a typed `CodingError` (`code: "unavailable"`) naming the evaluator's own code, which `repository/InvestigateStep` retains as an errored step result rather than as a step that looked and found nothing.
