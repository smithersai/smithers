...
This is the implementation and verification contract for [PRODUCT.md](PRODUCT.md), with the interaction design in [DESIGN.md](DESIGN.md). Implementation and production evidence below are updated through **2026-09-17 01:50 UTC**. The [release ledger](implementation/release-20260916.md) records exact deployed identities, failures, recovery and remaining gates; concurrent source changes are not deployment evidence.
...
| AI-check reuse from Artsy | Generic AI checks implemented; focused integration review underway | The setup card already edits AI rules, scope and report/required policy; the repository job executes those checks. [Artsy audit](research/artsy-checks-audit.md) includes current-framework declaration/plan and focused scripted-model checks. Required-finding scoring, live rubric quality and the complete deployed AI-check path remain under review. |
