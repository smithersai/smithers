...
Jev answers an AI check first. [`jev-checks.ts`](../../flows/repository/jev-checks.ts) splits the candidate diff into hunks and asks Jev one boolean per (rule, hunk), batched 64 states at a time: does this hunk violate the rule as stated? A probability at or above 0.8 flags the hunk, one at or below 0.2 clears it, and anything between is indecisive. Every hunk decisive and none flagged is a pass, a decisive flag is a fail whose finding cites the hunk's file and its first changed candidate line with the maintainer's own rule text as the message, and one indecisive hunk makes the whole check uncertain. Only an uncertain verdict reaches the `repository/checker` seat, which then decides the check unchanged, so a decisive pass or fail costs no frontier call. A host with no `AI_GATEWAY_API_KEY` installs an evaluator that refuses every evaluation, which reads as indecisive, so the seat keeps deciding every check exactly as it did before. The retained row records `decidedBy` as `jev` or `seat`.
