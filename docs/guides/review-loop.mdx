...
The implement-review loop is the most important pattern in Smithers. It wraps implementation in a `<Loop>` with validation and multi-agent review, catching issues early and forcing agents to fix them before moving on.
...
- [Loop Component](/components/ralph) -- All Loop props and iteration semantics.
