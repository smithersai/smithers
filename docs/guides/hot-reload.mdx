...
Smithers supports Hot Module Replacement (HMR) for workflows. When enabled, the engine watches your source files and reloads the workflow on save. In-flight tasks continue uninterrupted; only newly scheduled tasks use the updated code.

This is especially useful for long-running workflows where you want to tweak prompts, adjust agent configuration, or restructure the task graph without losing hours of accumulated state.
...
Hot reload and [resumability](/guides/resumability) work together. You can:
