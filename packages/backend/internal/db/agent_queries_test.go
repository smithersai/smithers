...
// -----------------------------------------------------------------------------
// Tickets 0115 + 0118: denormalized repository_id / session_id.
// -----------------------------------------------------------------------------

// TestAgentMessagesParts_DenormBackfill_PopulatesMissingColumns simulates the
// pre-migration state (rows without repository_id / session_id on
// agent_messages / agent_parts) and runs the backfill SQL from
// db/migrations/000035_add_agent_messages_parts_denorm.sql. After the
// backfill runs, every row must have its denormalized FK-chain parent values
// populated.
//
// This protects the *backfill* leg of the migration. The rollforward leg
// (column add + not-null + fkey) is covered by the schema itself being
// applied in TestMain.
func TestAgentMessagesParts_DenormBackfill_PopulatesMissingColumns(t *testing.T) {
	q, pool := newQueries(t)
	ctx := context.Background()

	userID := mustCreateUser(t, pool, "denorm-backfill-user")
	repoID := mustCreateRepo(t, pool, userID, "denorm-backfill-repo")

	sessionID := "ba01f111-aaaa-bbbb-cccc-ddddeeeeffff"
	_, err := q.CreateAgentSession(ctx, CreateAgentSessionParams{
		ID:           sessionID,
		RepositoryID: repoID,
		UserID:       userID,
	...
	// Step 1: within the test transaction, relax the NOT NULL constraints
	// and drop the FKs so we can simulate "old rows created before the
	// migration". (The migration itself adds the column as nullable first.)
	_, err = pool.Exec(ctx, `
		ALTER TABLE agent_messages ALTER COLUMN repository_id DROP NOT NULL;
		ALTER TABLE agent_parts    ALTER COLUMN repository_id DROP NOT NULL;
		ALTER TABLE agent_parts    ALTER COLUMN session_id    DROP NOT NULL;
	`)
	require.NoError(t, err)

	// Step 2: insert "legacy" rows with NULL denorm fields, bypassing the
	// sqlc-generated INSERT (which now requires the column).
	var messageID int64
	err = pool.QueryRow(ctx, `
		INSERT INTO agent_messages (session_id, role, sequence)
		VALUES ($1, 'user', 0)
		RETURNING id
	`, sessionID).Scan(&messageID)
	require.NoError(t, err)

	_, err = pool.Exec(ctx, `
		INSERT INTO agent_parts (message_id, part_index, part_type, content)
		VALUES ($1, 0, 'text', '{"text":"legacy"}'::jsonb)
	`, messageID)
	require.NoError(t, err)

	// Sanity: the legacy rows are NULL on denorm fields right now.
	var preMsgRepo pgtype.Int8
	var prePartRepo pgtype.Int8
	var prePartSession pgtype.Text
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT repository_id FROM agent_messages WHERE id = $1`, messageID).Scan(&preMsgRepo))
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT repository_id, session_id FROM agent_parts WHERE message_id = $1`, messageID).
		Scan(&prePartRepo, &prePartSession))
	require.False(t, preMsgRepo.Valid, "legacy agent_messages.repository_id starts NULL")
	require.False(t, prePartRepo.Valid, "legacy agent_parts.repository_id starts NULL")
	require.False(t, prePartSession.Valid, "legacy agent_parts.session_id starts NULL")

	// Step 3: run the exact backfill statements from migration 000035.
	_, err = pool.Exec(ctx, `
	...
	// Step 4: verify both rows now carry the denormalized values pointing
	// to the session's repository.
	var postMsgRepo int64
	var postPartRepo int64
	var postPartSession string
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT repository_id FROM agent_messages WHERE id = $1`, messageID).Scan(&postMsgRepo))
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT repository_id, session_id FROM agent_parts WHERE message_id = $1`, messageID).
		Scan(&postPartRepo, &postPartSession))

	assert.Equal(t, repoID, postMsgRepo,
		"ticket 0115 backfill: agent_messages.repository_id must match parent session")
	assert.Equal(t, repoID, postPartRepo,
		"ticket 0118 backfill: agent_parts.repository_id must match parent session")
	assert.Equal(t, sessionID, postPartSession,
		"ticket 0118 backfill: agent_parts.session_id must match parent session id")
}

// TestAgentMessages_CreateWithNextSequence_PopulatesRepositoryID asserts that
// the transactional append path's CTE insert (ticket 0115) takes
// repository_id from the locked parent session row, not from a mutable arg.
