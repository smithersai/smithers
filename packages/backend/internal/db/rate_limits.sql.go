...
const consumeSearchRateLimitToken = `-- name: ConsumeSearchRateLimitToken :one
WITH input AS (
    SELECT
        $1::text AS scope,
...
type ConsumeSearchRateLimitTokenParams struct {
...
type ConsumeSearchRateLimitTokenRow struct {
	Allowed         bool      `json:"allowed"`
	RemainingTokens float64   `json:"remaining_tokens"`
	NowAt           time.Time `json:"now_at"`
}

func (q *Queries) ConsumeSearchRateLimitToken(ctx context.Context, arg ConsumeSearchRateLimitTokenParams) (ConsumeSearchRateLimitTokenRow, error) {
	row := q.db.QueryRow(ctx, consumeSearchRateLimitToken,
		arg.Scope,
		arg.PrincipalKey,
		arg.Capacity,
		arg.RefillPerSecond,
	...
	var i ConsumeSearchRateLimitTokenRow
	err := row.Scan(&i.Allowed, &i.RemainingTokens, &i.NowAt)
	return i, err
}
...
func (q *Queries) DeleteAllRateLimits(ctx context.Context) error {
	_, err := q.db.Exec(ctx, deleteAllRateLimits)
	return err
}

const deleteExpiredSearchRateLimits = `-- name: DeleteExpiredSearchRateLimits :exec
DELETE FROM search_rate_limits
WHERE updated_at < $1::timestamptz
`

func (q *Queries) DeleteExpiredSearchRateLimits(ctx context.Context, cutoffAt time.Time) error {
	_, err := q.db.Exec(ctx, deleteExpiredSearchRateLimits, cutoffAt)
	return err
}
