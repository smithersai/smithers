...
func TestLegacyFinalKeyCapabilityHorizonFailsClosedAndCanOnlyExtend(t *testing.T) {
	...
	shortenErr := mustExpectQueryError(t, tx, func(spQ *Queries) error {
		_, updateErr := spQ.db.Exec(ctx, `
			UPDATE storage_legacy_capability_horizons
	...
	lateExtensionErr := mustExpectQueryError(t, tx, func(spQ *Queries) error {
		_, updateErr := spQ.db.Exec(ctx, `
			UPDATE storage_legacy_capability_horizons
	...
	deleteErr := mustExpectQueryError(t, tx, func(spQ *Queries) error {
		_, removeErr := spQ.db.Exec(ctx, `
...
func TestStorageDeletionQueueActiveCheckUsesLFSAllocationIdentity(t *testing.T) {
	...
	require.NoError(t, err)
	planRows, err := tx.Query(ctx, `EXPLAIN (COSTS OFF) `+isStorageDeletionObjectActive,
		repoID, fmt.Sprintf("lfs:%d:%s", repoID, oid))
	require.NoError(t, err)
	...
	assert.NotContains(t, plan, "Seq Scan on lfs_objects")
	assert.NotContains(t, plan, "Seq Scan on lfs_upload_reservations")
	assert.Contains(t, plan, "repository_id_oid_key",
		"active LFS checks must use repository+OID indexes rather than formatted-key scans")
}
