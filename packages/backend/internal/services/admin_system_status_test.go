...
type fakeStatusRuntimeStore struct {
	...
	sessionsErr    error
	oldestErr      error
	queue          WorkflowTaskQueueMetrics
	queueErr       error
}
...
func (f *fakeStatusRuntimeStore) GetWorkflowTaskQueueMetrics(context.Context) (WorkflowTaskQueueMetrics, error) {
...
func healthyStatusConfig() AdminSystemStatusServiceConfig {
	return AdminSystemStatusServiceConfig{
		...
		Runtime: &fakeStatusRuntimeStore{
			...
			activeSessions: 3,
			oldestSession:  120.5,
			queue:          WorkflowTaskQueueMetrics{Depth: 7, OldestAgeSeconds: 31.25},
		},
