...
func TestAdminStopWorkspaceRetainsRowAndEndsSessions(t *testing.T) {
	...
	deleted := 0
	var gauge float64
	lifecycle := newWorkspaceServiceForTests(q, WithWorkspaceSandboxClient(&mockWorkspaceSandboxVMClient{deleteVMFn: func(_ context.Context, vm string) error {
		...
		deleted++
		return nil
	}}), WithWorkspaceSandboxMetrics(&mockSandboxMetricsRecorder{addActiveVMsFn: func(_ string, delta float64) { gauge += delta }}))
	_, err = lifecycle.StopWorkspace(ctx, id, repo, user+1)
	require.Error(t, err)
	require.Zero(t, deleted)
	admin := NewAdminManageService(q, nil, lifecycle)
	ctx = ContextWithAdminAuditActor(ctx, AdminAuditActor{UserID: user, Username: owner})
