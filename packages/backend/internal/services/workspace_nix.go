...
func (s *WorkspaceService) resolveWorkspaceImage(ctx context.Context, repositoryID int64, kind string) (db.SandboxEnvironmentImage, error) {
	if s.environmentImages == nil {
		return db.SandboxEnvironmentImage{}, pkgerrors.EnvironmentImageUnavailable("kind " + normalizeWorkspaceKind(kind) + " workspaces need a registered NixOS environment image; this deployment has no image registry")
	}
...
func (s *WorkspaceService) applyNixGuest(req *sandbox.CreateRequest, image db.SandboxEnvironmentImage, wantSnapshot bool, snapshotID string) {
...
// failed.
func (s *WorkspaceService) nixGoldenSnapshotFor(ctx context.Context, image db.SandboxEnvironmentImage) string {
...
func (s *WorkspaceService) NixBakeVMRequest(image db.SandboxEnvironmentImage) sandbox.CreateRequest {
...
func (s *WorkspaceService) recordWorkspaceEnvironment(ctx context.Context, workspaceID string, image db.SandboxEnvironmentImage) {
