...
const (
	...
	// AlertRemediationTriggerEvent is the webhook trigger event dispatched for
	// remediation runs; remediate.tsx subscribes via on.webhook("monitoring_alert").
	AlertRemediationTriggerEvent = "monitoring_alert"
)
...
func (w *AlertRemediationWorker) processJob(ctx context.Context, job db.AlertRemediationJob) error {
	...
	results, err := w.dispatcher.DispatchForEvent(ctx, DispatchForEventInput{
		RepositoryID:         w.repositoryID,
