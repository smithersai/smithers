...
func incidentMutationParams(input AdminIncidentBulkInput, actor AdminAuditActor) db.AdminMutateAlertIncidentsParams {
...
// AuditService supplies the established event encoding. Capture its write
...
func logIncidentAudit(ctx context.Context, q AuditQueries, actor AdminAuditActor, input AdminIncidentBulkInput, id *int64, bulk bool, affected int64) error {
	event := "admin.incident."
	if bulk {
		event += "bulk_"
	}
	writer := &incidentAuditWriter{AuditQueries: q}
	NewAuditService(writer).Log(ctx, AuditEvent{
