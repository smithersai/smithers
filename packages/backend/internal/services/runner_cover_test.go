...
func TestRunner_Cov_CheckRunAnnotationsAndUpdate(t *testing.T) {
	...
	assert.Equal(t, GitHubCheckRunAnnotation{
		Path:            "src/app.go",
		StartLine:       7,
		EndLine:         7,
...
func TestRunner_Cov_StatusMappingsAndMetricObservation(t *testing.T) {
	...
	assert.Equal(t, "Workflow hit an infrastructure error", workflowRunStatusDescription("error"))
	assert.Equal(t, "Workflow status: waiting", workflowRunStatusDescription("waiting"))
	...
	metrics := &runnerCovMetrics{}
	observeWorkflowRunCompletion(metrics, db.WorkflowRun{
		ID:        700,
		Status:    "running",
	...
	observeWorkflowRunCompletion(metrics, db.WorkflowRun{ID: 701, Status: "success", CreatedAt: time.Now()}, "failure")
