...
type lfsStorageDeletionAllocationQuerier interface {
	HasStorageDeletionAllocation(ctx context.Context, arg db.HasStorageDeletionAllocationParams) (bool, error)
}
	...
	resolveAdditionalBytes := func(ctx context.Context) (int64, error) {
		...
		for _, oid := range oids {
			...
			if allocationStore, ok := s.queries.(lfsStorageDeletionAllocationQuerier); ok {
				queued, queueErr := allocationStore.HasStorageDeletionAllocation(ctx, db.HasStorageDeletionAllocationParams{
					RepositoryID:  repositoryID,
