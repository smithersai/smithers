...
func WithRunnerGitHubCheckRunService(service GitHubCheckRunService) RunnerServiceOption {
	return func(s *runnerService) {
...
func WithRunnerGitHubInstallationResolver(resolver GitHubRepositoryInstallationResolver) RunnerServiceOption {
	return func(s *runnerService) {
...
func WithRunnerMetrics(metrics WorkflowRunMetricsObserver) RunnerServiceOption {
	return func(s *runnerService) {
		s.metrics = metrics
	}
}

func WithRunnerSecretInjector(injector *SecretInjector) RunnerServiceOption {
	return func(s *runnerService) {
...
func (s *runnerService) Terminate(ctx context.Context, runnerID int64) error {
	...
	if tx, txQueries, transactional, txErr := beginWorkflowQueryTx(ctx, s.queries); transactional {
		if txErr != nil {
...
func (s *runnerService) GetTaskRuntimeEnvironment(ctx context.Context, taskID int64) (map[string]string, error) {
	...
	if len(secretNames) > 0 {
		...
		env[secretEnvKeysRuntimeMarker] = strings.Join(secretNames, ",")
	}
...
func workflowTaskSecretAllowlist(payload []byte) (map[string]struct{}, bool, error) {
	...
	if len(*envelope.SecretNames) > maxInjectedEnvEntries {
...
func (s *runnerService) StreamEvents(ctx context.Context, input RunnerStreamEventsInput) error {
	...
	for i := range logEvents {
		logEvents[i].text = RedactSecretValues(redactionEnv, logEvents[i].text)
	}
...
func (s *runnerService) CompleteTask(ctx context.Context, input RunnerCompleteTaskInput) error {
	...
	if runErr == nil {
		observeWorkflowRunCompletion(s.metrics, run, runStatus)
	}
	if isTerminalWorkflowRunStatus(runStatus) {
		...
		revokeWorkflowRunCredentials(ctx, s.queries, workflowRunID, repoID)
	}
	if s.commitStatusWriter != nil && isTerminalWorkflowRunStatus(runStatus) {
		if _, err := s.commitStatusWriter.UpdateCommitStatusForWorkflowRun(ctx, workflowRunID, runStatus, workflowRunStatusDescription(runStatus), ""); err != nil {
			middleware.LoggerWithWorkflowRun(ctx, workflowRunID).
				Error("failed to update commit status for workflow run", "status", runStatus, "error", err)
		}
	}
	if runErr == nil && s.checkRunService != nil && isTerminalWorkflowRunStatus(runStatus) {
	...
	notifyWorkflowRunEvent(ctx, s.queries, workflowRunID, "runner.complete_task")
...
func (s *runnerService) completeTaskWithTransaction(
	...
	if err := lockWorkflowRun(ctx, tx, task.WorkflowRunID); err != nil {
		if stdErrors.Is(err, pgx.ErrNoRows) {
	...
	if isTerminalWorkflowRunStatus(runStatus) {
		observeWorkflowRunCompletion(s.metrics, run, runStatus)
		s.transitionAgentSessionForTerminalWorkflowRun(ctx, workflowRunID, runStatus)
		revokeWorkflowRunCredentials(ctx, s.queries, workflowRunID, run.RepositoryID)
...
func acknowledgeTerminalRunnerTaskWithTransaction(
	...
	if err := lockWorkflowRun(ctx, tx, workflowRunID); err != nil {
		if stdErrors.Is(err, pgx.ErrNoRows) {
			return pkgerrors.Conflict("workflow run not found")
		}
		return pkgerrors.Internal("failed to lock workflow run")
...
func (s *runnerService) transitionAgentSessionForTerminalWorkflowRun(ctx context.Context, workflowRunID int64, runStatus string) {
	...
	eventPayload, _ := json.Marshal(AgentSessionEvent{
...
func (s *runnerService) collectCheckRunAnnotationsFromLogs(ctx context.Context, runID int64) ([]GitHubCheckRunAnnotation, error) {
	afterID := int64(0)
...
func parseCheckRunAnnotationLine(line string) (GitHubCheckRunAnnotation, bool) {
...
func (s *runnerService) progressDependenciesWith(ctx context.Context, queries RunnerQuerier, workflowRunID int64) error {
	for {
		...
		for _, bt := range blockedTasks {
			...
			if ifExpr != "" {
				shouldRun, evalErr := EvaluateIfExpression(ifExpr, parseTriggerEventFromPayload(bt.Payload), needsResults)
				if evalErr != nil {
...
func parseTriggerEventFromPayload(payload json.RawMessage) TriggerEvent {
	var p struct {
...
func (s *runnerService) dispatchTriggeredWorkflowRuns(ctx context.Context, run db.WorkflowRun, runStatus string) error {
	...
	_, err = s.workflowDispatcher.DispatchForEvent(ctx, DispatchForEventInput{
