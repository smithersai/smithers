...
func TestExtractAgentToken(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			authHeader: "Bearer smithers_agent_abcdef0123456789abcdef0123456789abcdef01",
			wantToken:  "smithers_agent_abcdef0123456789abcdef0123456789abcdef01",
			wantOk:     true,
		},
		{
		...
		{
			name:       "wrong prefix - smithers_ instead of smithers_agent_",
			authHeader: "Bearer smithers_abcdef0123456789abcdef0123456789abcdef01",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "wrong prefix - no prefix",
			authHeader: "Bearer abcdef0123456789abcdef0123456789abcdef01",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "wrong tail length - too short",
			authHeader: "Bearer smithers_agent_abcdef01",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "wrong tail length - too long",
			authHeader: "Bearer smithers_agent_abcdef0123456789abcdef0123456789abcdef0123",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "non-hex character in tail - uppercase",
			authHeader: "Bearer smithers_agent_ABCDEF0123456789abcdef0123456789abcdef01",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "non-hex character in tail - special char",
			authHeader: "Bearer smithers_agent_abcdef0123456789abcdef0123456789abcdefg1",
			wantToken:  "",
			wantOk:     false,
		},
		{
			name:       "three part auth header",
			authHeader: "Bearer smithers_agent_abcdef0123456789abcdef0123456789abcdef01 extra",
			wantToken:  "",
			wantOk:     false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotToken, gotOk := extractAgentToken(req)
			if gotToken != tt.wantToken {
				t.Errorf("extractAgentToken() gotToken = %v, want %v", gotToken, tt.wantToken)
			}
			if gotOk != tt.wantOk {
				t.Errorf("extractAgentToken() gotOk = %v, want %v", gotOk, tt.wantOk)
			}
		})
	}
}
