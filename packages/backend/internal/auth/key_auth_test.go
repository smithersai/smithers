...
func TestKeyAuthVerifier_Verify(t *testing.T) {
	...
	hash := keyAuthTextHash([]byte(message))
	sig, err := testSign(hash, privateKey)
	require.NoError(t, err)
	signature := testHexEncode(sig)
	legacySig := append([]byte(nil), sig...)
	legacySig[keyAuthRecoveryIDOffset] += 27
	legacySignature := testHexEncode(legacySig)
	invalidRecoverySig := append([]byte(nil), sig...)
	invalidRecoverySig[keyAuthRecoveryIDOffset] = 2
	invalidRecoverySignature := testHexEncode(invalidRecoverySig)
	...
	otherKey, err := testKeyFromHex("6cbed15c793ce57650b9877cf6fa156fbef513c4e6134f022a85b1ffdd59b2a1")
	require.NoError(t, err)
	otherHash := keyAuthTextHash([]byte(message))
	otherSig, err := testSign(otherHash, otherKey)
	require.NoError(t, err)
	signerMismatchSignature := testHexEncode(otherSig)
...
func TestKeyAuthVerifier_Verify_URIAndChainBinding(t *testing.T) {
	...
	privateKey, err := testKeyFromHex("4f3edf983ac636a65a842ce7c78d9aa706d3b113bce036f9f6e2b7fbc1f8f9af")
	require.NoError(t, err)
	address := keyAuthAddress(privateKey.PubKey())
	...
	buildAndSign := func(t *testing.T, uri, chainID string) (string, string) {
		...
		sig, err := testSign(keyAuthTextHash([]byte(msg)), privateKey)
		require.NoError(t, err)
		return msg, testHexEncode(sig)
	}
