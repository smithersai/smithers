...
type AdminSystemStatusServiceConfig struct {
	...
	Runtime      services.RuntimeMetricsStore
	...
	// services.CanaryStaleAfter overrides cadence-specific windows when positive.
...
func (s *AdminSystemStatusService) canaries(ctx context.Context, record func(string, error)) AdminSystemStatusCanaries {
	...
	for _, result := range results {
		...
		switch services.ClassifyCanaryStatus(result.Status) {
		case "passing":
			tally.Passing++
		case "failing":
			tally.Failing++
		...
		if window <= 0 {
			window = services.CanaryFreshnessWindow(result.Suite, result.TestName)
		}
		if services.CanaryIsStale(result.ReportedAt, now, window) {
			tally.Stale++
		}
	}
