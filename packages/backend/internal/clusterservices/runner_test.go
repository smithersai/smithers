...
type mockRunnerQuerier struct {
	...
	// asserting on services.RevokeWorkflowRunCredentials' duck-typed call. Left unset in
	// most tests, in which case the type assertion in services.RevokeWorkflowRunCredentials
...
type mockRunnerCheckRunService struct {
	updateFn func(
		...
		repo string,
		checkRunID int64,
		update services.GitHubCheckRunUpdate,
	) (services.GitHubCheckRunResult, error)
...
func (m *mockRunnerCheckRunService) PostCheckRun(
	...
	owner string,
	repo string,
	input services.GitHubCheckRunInput,
) (services.GitHubCheckRunResult, error) {
	return services.GitHubCheckRunResult{}, nil
}
...
func (m *mockRunnerCheckRunService) UpdateCheckRun(
	...
	return services.GitHubCheckRunResult{ID: checkRunID}, nil
}
...
type mockRunnerWorkflowDispatcher struct {
	...
	calls      []services.DispatchForEventInput
}

func (m *mockRunnerWorkflowDispatcher) DispatchForEvent(ctx context.Context, input services.DispatchForEventInput) ([]services.WorkflowRunResult, error) {
...
func TestRunnerService_GetTaskRuntimeEnvironment_NoSecretsOmitsMarker(t *testing.T) {
	...
	}, WithRunnerSecretInjector(services.NewSecretInjector(&mockSecretInjectionQuerier{}, webhook.NoopSecretCodec{})))
...
func TestRunnerService_CompleteTask_UpdatesGitHubCheckRunWithLogAnnotations(t *testing.T) {
	...
	annotationsByMessage := make(map[string]services.GitHubCheckRunAnnotation, len(call.update.Output.Annotations))
