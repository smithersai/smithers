...
func TestRunnerServiceIntegration_CompleteTask_UpdatesLinkedCommitStatus(t *testing.T) {
	...
	commitStatusService := services.NewCommitStatusService(queries)
	workflowRunService := services.NewWorkflowRunService(queries, services.WithWorkflowRunCommitStatusWriter(commitStatusService))
	results, err := workflowRunService.DispatchForEvent(ctx, services.DispatchForEventInput{
		RepositoryID: repoID,
