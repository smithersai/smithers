...
func TestServer_H_PublicKeyHandlerBannedAndThrottledMetrics(t *testing.T) {
	...
	credential := "key:SHA256:" + base64.RawStdEncoding.EncodeToString(keyHash[:])
	bannedLimiter.RecordFailure("203.0.113.8", credential)
	bannedServer := &Server{
	...
	assert.Equal(t, AuthLimitAllowed, throttledLimiter.Check("203.0.113.9", credential))
