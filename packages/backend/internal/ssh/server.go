...
func (s *Server) publicKeyHandler(ctx ssh.Context, key ssh.PublicKey) bool {
	...
	if s.AuthLimiter != nil {
		switch s.AuthLimiter.Check(remoteIP) {
		case AuthLimitBanned:
			slog.Warn("ssh auth denied: ip banned", "remote_ip", remoteIP)
			if s.Metrics != nil {
				s.Metrics.AuthAttempts.WithLabelValues("banned").Inc()
			}
			return false
		case AuthLimitThrottled:
			slog.Warn("ssh auth denied: ip throttled", "remote_ip", remoteIP)
			if s.Metrics != nil {
	...
	if s.Queries == nil {
		...
		if s.AuthLimiter != nil {
			s.AuthLimiter.RecordFailure(remoteIP)
		}
		if s.Metrics != nil {
	...
	if workspaceLogin {
		...
		if principal.IsDeployKey || s.WorkspaceBridge == nil || s.WorkspaceBridge.Validate(ctx, workspace) != nil {
			if s.AuthLimiter != nil {
				s.AuthLimiter.RecordFailure(remoteIP)
			}
			if s.Metrics != nil {
	...
	if s.AuthLimiter != nil {
		s.AuthLimiter.RecordSuccess(remoteIP)
	}
	if s.Metrics != nil {
...
func (s *Server) passwordHandler(ctx ssh.Context, password string) bool {
	...
	if s.AuthLimiter != nil {
		switch s.AuthLimiter.Check(remoteIP) {
		case AuthLimitBanned, AuthLimitThrottled:
			return false
		}
	}
	if err := s.WorkspaceBridge.Validate(ctx, workspace); err != nil {
		if s.AuthLimiter != nil {
			s.AuthLimiter.RecordFailure(remoteIP)
		}
		if s.Metrics != nil {
	...
	slog.Info("workspace ssh auth succeeded", "sandbox_id", workspace.SandboxID, "guest_user", workspace.User, "remote_ip", remoteIP)
	return true
}
...
func (s *Server) sessionHandler(sess ssh.Session) {
	...
	if workspace, ok := sess.Context().Value(workspaceAccessKey).(WorkspaceAccess); ok {
		if s.WorkspaceBridge == nil {
			_, _ = fmt.Fprintln(sess.Stderr(), "ERROR: workspace SSH is unavailable")
			_ = sess.Exit(1)
			return
		}
...
func (s *Server) authorizeDeployKeyForRepository(ctx context.Context, principal sshPrincipal, repository db.Repository, mode services.AccessMode) (sshPrincipal, error) {
	...
	if err != nil {
		...
		return sshPrincipal{}, apierrors.Internal("failed to resolve deploy key")
	}
	return s.authorizeResolvedDeployKey(ctx, principal, key, repository, mode)
}
...
func (s *Server) resolveRepositoryByName(ctx context.Context, owner, repo string) (db.Repository, error) {
	...
	if err != nil {
		...
		return db.Repository{}, apierrors.Internal("failed to resolve repository")
	}
...
func (s *Server) resolveDeployKeyForRepo(ctx context.Context, owner, repo, fingerprint string) (db.DeployKey, db.Repository, error) {
	...
	if err != nil {
		...
		return db.DeployKey{}, db.Repository{}, apierrors.Internal("failed to resolve deploy key")
	}
