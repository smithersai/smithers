...
import { nativeRefusal } from "./refusalFixtures.ts"
...
describe("the native host's own failure registry", () => {
  test("shares no code with plue's or the Worker's, which is what lets one string name its author", async () => {
   ...
   */
  test("reserves its namespace: no code of plue's or the Worker's may wear the prefix", async () => {
   ...
   */
  test("names the collisions it was written for, so the prefix is not mistaken for decoration", async () => {
  ...
  test("answers the same three questions per row as plue's and the Worker's", async () => {
   ...
   * infra.
   */
  test("never claims Smithers' own infra failed, because none of it is ours to run out of", async () => {
    /*
    ...
    for (const code of NATIVE_ROUTE_CODES) {
      const refusal = await nativeRefusal(code, "x")
...
describe("a native-host refusal as it reaches the app", () => {
  test("carries the host's code, its documented fault, and origin=local", async () => {
    for (const code of NATIVE_ROUTE_CODES) {
      const entry = NATIVE_FAILURES[code]
   ...
   */
  test("is local by its code alone, with no origin stated", async () => {
  ...
  test("the host's envelope reads back with the table's status and fault", async () => {
    for (const code of NATIVE_ROUTE_CODES) {
      const entry = NATIVE_FAILURES[code]
      const built = await nativeRefusal(code, "nope")
      expect({ code: built.code, status: built.status, fault: built.fault, origin: built.origin }).toEqual({
  ...
  test("survives a round trip through a card's stored shape", async () => {
    const refusal = await nativeRefusal("node_missing", "No Node.js >= 26.4 was found for the smithers-build CLI.")
   ...
   */
  test("beats the status guess it used to get", async () => {
    expect(faultOfStatus(503)).toBe("infra")
    expect((await nativeRefusal("node_missing", "x")).fault).toBe("dependency")
    expect(faultOfStatus(501)).toBe("bug")
    expect((await nativeRefusal("not_implemented", "x")).fault).toBe("user")
  })

  test("is never retried on a timer unless the host said to wait and said how long", async () => {
    for (const code of NATIVE_ROUTE_CODES) {
      const refusal = await nativeRefusal(code, "x")
  ...
  test("tells the chat model the fault class for a native code too, through the string channel", async () => {
