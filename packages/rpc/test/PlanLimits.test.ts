...
import { refusalFromStored, refusalOf, storedRefusal } from "../src/Refusal.ts"
...
test("plan-limit metadata and upgrade copy survive the shared refusal boundary", () => {
  const body = {
    ...
    plan_key: "free",
    limit_kind: "sandbox_hours_per_day",
    upgrade_plan_key: "pro"
  }
  const refusal = refusalOf({
    body: machineReadableRefusal(JSON.stringify(body)),
    status: 402,
    message: "Daily sandbox hours exhausted."
  })
