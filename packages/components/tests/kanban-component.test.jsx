...
describe("Kanban", () => {
    workflowTest("happy path processes every item through all columns", async () => {
        ...
        const workflow = smithers((ctx) => {
            ...
            const doneIds = new Set(reviewRows
                .filter((row) => row.next === "done")
                .map((row) => row.itemId));
            const remaining = tickets.filter((ticket) => !doneIds.has(ticket.id));
            return (<Workflow name="kanban-happy">
          <Kanban id="board" columns={[
                    {
                        name: "triage",
                        agent: triageAgent,
                ...
                ]} useTickets={() => remaining} until={remaining.length === 0} maxIterations={3} onComplete={outputs.board}>
            {{
                    done: [...doneIds].sort(),
    ...
    workflowTest("transient worker failure retries and the board still completes", async () => {
        ...
        const workflow = smithers((ctx) => {
            ...
            const doneIds = new Set(reviewRows
                ...
                .map((row) => row.itemId));
            const remaining = tickets.filter((ticket) => !doneIds.has(ticket.id));
            return (<Workflow name="kanban-retry">
          <Kanban id="board" columns={[
                    {
                        name: "triage",
                        agent: triageAgent,
