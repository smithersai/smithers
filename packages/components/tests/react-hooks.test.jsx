...
describe("React hooks e2e", () => {
     ...
     */
        function StateUpdater({ iteration }) {
            ...
            return (<Task id="state-task" output={outputs.outputA} agent={{
                    id: "echo",
                    tools: {},
        ...
        const workflow = smithers((ctx) => (<Workflow name="usestate-setter">
    ...
    test("useEffect fires and produces observable side effects", async () => {
        ...
        function EffectComponent() {
            ...
            return (<Task id="effect-task" output={outputs.outputA} agent={{
                    id: "echo",
                    tools: {},
        ...
        const workflow = smithers((_ctx) => (<Workflow name="useeffect-side-effect">
        <EffectComponent />
      </Workflow>));
        const result = await Effect.runPromise(runWorkflow(workflow, { input: {} }));
        expect(result.status).toBe("finished");
    ...
    test("useRef persists across re-renders in a Ralph loop", async () => {
        ...
        function RefTracker() {
            ...
            return (<Task id="ref-task" output={outputs.outputA} agent={{
                    id: "echo",
                    tools: {},
        ...
        const workflow = smithers((_ctx) => (<Workflow name="useref-test">
     ...
     */
        function IterationTracker({ iteration }) {
            ...
            return (<Task id="iter-task" output={outputs.outputA} agent={{
                    id: "echo",
                    tools: {},
        ...
        const workflow = smithers((ctx) => (<Workflow name="effect-deps">
        <Ralph id="loop" maxIterations={3}>
...
describe("TanStack Query e2e", () => {
     ...
     */
        function DataTask({ outputs: o }) {
            ...
            return (<Task id="query-task" output={o.outputA} agent={{
                    id: "echo",
                    tools: {},
                    generate: async () => {
        ...
        const workflow = smithers((_ctx) => (<Workflow name="tanstack-e2e">
        <QueryComponent />
      </Workflow>));
        const result = await Effect.runPromise(runWorkflow(workflow, { input: {} }));
        expect(result.status).toBe("finished");
