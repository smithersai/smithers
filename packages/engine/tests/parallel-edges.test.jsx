...
describe("<Parallel> edge maxConcurrency semantics", () => {
    test("extract: non-positive => undefined (unbounded); fractional floored", async () => {
        ...
        // 0 => undefined
        let res = await renderer.render(<Workflow name="par-edge-0">
        ...
        // -1 => undefined
        res = await renderer.render(<Workflow name="par-edge-neg">
        <Parallel maxConcurrency={-1}>{base}</Parallel>
      </Workflow>);
        expect(res.tasks[0].parallelMaxConcurrency).toBeUndefined();
        expect(res.tasks[1].parallelMaxConcurrency).toBeUndefined();
        // 2.9 => 2 (floor)
        res = await renderer.render(<Workflow name="par-edge-frac">
     ...
     */
        async function runWith(mc) {
            peak = 0;
            const wf = smithers((_ctx) => (<Workflow name={`par-edge-run-${String(mc)}`}>
          <Parallel maxConcurrency={mc}>
            {Array.from({ length: 6 }, (_, i) => (<Task key={`p${i}`} id={`p${mc}-${i}`} output={outputs.outputC} agent={agent}>
                run task
              </Task>))}
          </Parallel>
        </Workflow>));
    ...
    test("engine: fractional floors to integer cap", async () => {
        ...
        const wf = smithers((_ctx) => (<Workflow name="par-frac">
        <Parallel maxConcurrency={2.9}>
          {Array.from({ length: 5 }, (_, i) => (<Task key={`pf${i}`} id={`pf${i}`} output={outputs.outputC} agent={agent}>
              run task
            </Task>))}
        </Parallel>
      </Workflow>));
