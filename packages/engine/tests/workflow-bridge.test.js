 ...
 */
const executeBridge = (adapter, runId, desc, legacyExecuteTaskFn, options = {}) => executeTaskBridge(adapter, {}, runId, desc, new Map(), null, new EventBus({}), toolConfig, "bridge-contract", false, options.signal, undefined, undefined, undefined, legacyExecuteTaskFn);
describe("executeTaskBridge", () => {
    ...
    test("rejects when no legacy executor is provided", async () => {
        ...
        const adapter = makeAdapter(() => []);
        await expect(executeBridge(adapter, "run-missing-legacy", desc)).rejects.toThrow("legacyExecuteTaskFn must be provided");
    });
});
