...
| `decided`, `health`                                                                   | Fold a decision in: the dot on the title and a `health` card on a color change, sorted under the frame whose settlement or park produced the facts; `health` counts the Jev call when Jev answered and puts a failure on the card.          |
...
| Export                                                                                 | Signature                                                                                      | Meaning                                                                                                                                                                                                                                                                    |
| -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `State`, `Facts`, `toState`                                                            | schema, `State & { demandThisFrame, stoppedBy }`, `(facts) => State`                           | What Jev is sent (design section 3.1), and the two server-only facts the rule also reads.                                                                                                                                                                                  |
...
| `dots`, `colorOf`, `strip`, `dotted`, `retitle`                                        | title helpers                                                                                  | The dot per color, and how a title carries, loses, and keeps one across a rename; `strip` drops the U+FE0F the app writes after the dot; `retitle` drops a dot the rename arrives with and keeps the session's own, and an empty rename keeps the title.                   |
...
| `evaluatorLayer`, `evaluatorConfigured`, `ambientEnvironment`, `noGatewayKey`          | `(environment) => Layer<Evaluator>`, `(environment) => boolean`, `() => process.env`, `string` | Jev through the Vercel gateway under `evaluatorRetry` when `AI_GATEWAY_API_KEY` is set, else one that refuses with `noGatewayKey` and is not retried. `evaluatorConfigured` answers the same question before the layer is built, which is what the startup preflight asks. |
...
| `Service`                         | `{ prompt, abort, permission, update, status }`                                                                                                             | The composition; `update` applies a rename or an archive in order with the turn's writes. |
...
| `refusing`                         | `(source, rejected) => FlowBinding.Source`                                                                               | Settles a rejected call as `capability_refused` with `deniedMessage`.                                                                                                                                                        |
