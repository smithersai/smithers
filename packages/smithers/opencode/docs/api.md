...
| `evaluatorLayer`, `evaluatorConfigured`, `ambientEnvironment`, `noGatewayKey`          | `(environment) => Layer<Evaluator>`, `(environment) => boolean`, `() => process.env`, `string` | Jev through the Vercel gateway when `AI_GATEWAY_API_KEY` is set, else one that refuses with `noGatewayKey`. `evaluatorConfigured` answers the same question before the layer is built, which is what the startup preflight asks.                         |
