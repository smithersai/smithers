...
import * as PlanGraphBuildError from "@smthrs/plan/GraphBuildError"
...
describe("Graph module boundary", () => {
  it("delegates the diagnostic and every limit it no longer declares itself", () => {
    expect(Graph.GraphBuildError).toBe(PlanGraphBuildError.GraphBuildError)
    expect(Graph.GraphBuildErrorCode).toBe(PlanGraphBuildError.GraphBuildErrorCode)
    expect(Graph.isFatalDiagnostic).toBe(PlanGraphBuildError.isFatalDiagnostic)
  ...
  it("refuses an over-deep plan value through the extracted reflection", () => {
    ...
    expect(() => Graph.build(Flow.make({ body: () => Node.succeed(deep) })))
      .toThrow(expect.objectContaining({ _tag: "@smthrs/plan/GraphBuildError", code: "payload_too_deep" }))
  })
})
