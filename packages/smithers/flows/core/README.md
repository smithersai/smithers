...
| `Graph.build` on an invalid declaration                                              | records `GraphBuildError` in `Graph.diagnostics` with code `effect_outside_envelope`, `effect_mode_widening`, `effect_tier_widening`, `write_conflict`, `capability_outside_grant`, `duplicate_node`, or `missing_key_material`                                                                                                   |
| `Graph.build` on a malformed or oversized plan                                       | throws `GraphBuildError` with code `invalid_node`, `graph_too_deep`, `plan_too_large`, `payload_too_deep`, or `payload_too_large`                                                                                                                                                                                                 |
