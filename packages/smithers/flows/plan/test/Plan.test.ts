...
describe("PlanDiff.diff", () => {
  ...
  it.effect("compares a planned value in hand-built material by its reference", () =>
    Effect.gen(function*() {
      const compiled = yield* withCrypto(compile([draft("node")]))
      const upstream = Planned.make<{ readonly x: unknown; readonly y: unknown }>("upstream")
      const withBody = (body: unknown, digit: string): Plan.Plan => ({
        ...compiled,
        nodes: [{
          ...compiled.nodes[0]!,
          key: Schema.decodeUnknownSync(StoredKey)(`key1_${digit.repeat(64)}`),
          material: { ...compiled.nodes[0]!.material, body }
        }]
      })
      const same = PlanDiff.diff(withBody({ at: upstream.x }, "0"), withBody({ at: upstream.x }, "1"))
      expect(same.rekeyed[0]!.changed).toEqual([])
      const moved = PlanDiff.diff(withBody({ at: upstream.x }, "0"), withBody({ at: upstream.y }, "1"))
      expect(moved.rekeyed[0]!.changed).toEqual(["body"])
    }))
})
