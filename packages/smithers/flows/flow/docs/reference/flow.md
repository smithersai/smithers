...
| Name                 | Kind            | Summary                                                                                                                                                                                                                                                                                             |
| -------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Tier`               | Schema and type | The durability and retry semantics of an action: `"sealed"`, `"compensable"`, or `"irreversible"`. It IS `@smthrs/plan`'s `Effects.Tier`.                                                                                                                                                           |
...
| `BoundaryMode`       | Schema and type | How strictly an action's filesystem boundary is enforced: `"hard"` or `"expected"`. It IS `@smthrs/plan`'s `FileSet.BoundaryMode`.                                                                                                                                                                  |
...
| Name                  | Kind                 | Summary                                                                                                                                                                                                                                                                                                                       |
| --------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Effects`             | Schema and type      | The declared filesystem effects of a flow: `reads`, `writes`, optional `removes` (workspace-relative patterns, as on an action boundary), and `boundaryMode`. It IS `@smthrs/plan`'s `Plan.NodeEffects`, not a copy of it.                                                                                                    |
| `PlacementDirective`  | Type                 | Where a node runs. It IS `@smthrs/plan`'s `Placement.Placement`, the one typed model, carried unchanged until planning reads it.                                                                                                                                                                                              |
...
- **Type:** `Context.Key<Effects.Declaration, Effects.Declaration>`
- **Since:** `0.1.0`

The annotation key for the effect AUTHORITY a flow declares: the ceiling every step spliced beneath it must stay inside. It IS `@smthrs/plan`'s `Effects.Envelope`, identified by `"@smthrs/plan/Effects/Envelope"`, which `@smthrs/core` also publishes as `Annotations.Effects`, so a flow annotated for either graph builder is visible to the other. Its value is the one envelope model from `@smthrs/plan/Effects`, and `Flow.make`'s `effects` literal writes it.
...
- **Type:** `Context.Key<PlacementDirective, PlacementDirective>`
- **Since:** `0.1.0`

The annotation key for a flow's placement directive. It IS `@smthrs/plan`'s `Placement.Annotation`, identified by `"@smthrs/plan/Placement"`, which `@smthrs/core` also publishes as `Annotations.Placement`. Crossing between the two used to be a cast in `@smthrs/registry`.
