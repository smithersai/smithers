...
describe.skipIf(!jjInstalled)("NodeJj under a user config", () => {
  ...
  it.effect("returns clean change ids, diffs and roots with ui.color = always", () =>
    Effect.gen(function*() {
      ...
      const { changeId, commitId, operationId } = yield* jj.snapshot("first")
      expect(changeId).toMatch(/^[k-z]+$/)
      expect(commitId).toMatch(/^[0-9a-f]{40}$/)
      expect(operationId).toMatch(/^[0-9a-f]{128}$/)
      yield* Effect.promise(() => writeFile(join(repository, "note.txt"), "second\n"))
      const second = yield* jj.snapshot("second")
      const diff = yield* jj.diff(commitId, second.commitId)
