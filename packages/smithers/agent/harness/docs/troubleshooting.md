...
| `claim_unproven`       | Jev read the run's completion against the run's own record and the record does not support it, and the run had no bounce left to spend. The message carries both probabilities. Raise `claimCap` to give the run more frames to prove a claim in, or `0` to disarm the brake. |
