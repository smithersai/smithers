...
`@smthrs/memory` is one capability of the Smithers agent, and [`@smthrs/agent`](/api/agent) is the package that gives it that context. The agent package owns the agent loop, and it reaches memory in three places. `StandardFlows.memory` binds the `remember` and `recall` declarations defined here into the flows a model can call, so a model that writes a fact crosses the same journaled boundary as a model that reads a file. `Agent.Options.memory` accepts the frozen `{ rows, digest }` snapshot `Source.declared` builds, which becomes the opening context of a run; a judged run's relevance reading may withhold rows from it. And `@smthrs/agent/MemorySnapshotRecorder` implements the `SnapshotRecorder` port declared here, so a resumed run reads the snapshot its first attempt read instead of refetching live memory.
