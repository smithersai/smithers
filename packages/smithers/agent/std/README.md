...
| `Classify`           | `name`, `description`, `MAX_STATES`, `MAX_STATE_BYTES`, `CONCURRENCY`, `State`, `Questions`, `States`, `Input`, `Verdict`, `BatchResult`, `Output`, `effects`, `effectsFor`, `capabilities`, `flow`, `ask`, `askAll`, `run`, `curated` | Declares and runs typed Jev questions over JSON state, ad hoc or curated.      |
