...
const Delegate = Flow.make("safety/Delegate", {
...
const declaration = `import { Flow } from "@smthrs/core"
...
const withProject = <A, E>(
  ...
  Effect.gen(function*() {
    ...
    return yield* use(root).pipe(
      Effect.provide(
        Executable.layer({ delegates: [Delegate] }).pipe(
          Layer.provideMerge(
...
describe("a rebuild swaps a body without dropping the one it replaces", () => {
  it.effect("registers the new body before the previous scope closes", () => {
    ...
    return withProject(registrations, () =>
      Effect.gen(function*() {
        const refresh = yield* Executable.Refresh
        // The first refresh replaces the registration the startup layer holds,
        // which that layer's own scope owns; the second replaces one this
        // seam forked, which is the swap the guarantee is about.
        // Startup registered #1. The first rebuild registers #2 and holds its
        // scope; the second registers #3 and releases #2.
        expect((yield* refresh.flow("early"))._tag).toBe("Registered")
        expect((yield* refresh.flow("early"))._tag).toBe("Registered")
        ...
        expect(registrations.liveOnRegister).toEqual([0, 1, 2])
      }))
  }, 60_000)

  it.effect("keeps the previous entry and its live body when a registration dies", () => {
    const registrations = runtime({
      // The second registration is the one a rebuild performs. A body that
      // cannot register is the case this arm exists for.
      onRegister: (_tag, count) => count === 2 ? Effect.die("registration died") : Effect.void
    })
    return withProject(registrations, () =>
      Effect.gen(function*() {
        const catalog = yield* Executable.Catalog
        ...
        // it; the body registered before it was left alone.
        expect(registrations.events).toEqual(["register:early#1", "register:early#2", "release:early#2"])

        // And the host is not wedged. The next rebuild registers, then
        // releases the body it replaced.
        expect((yield* refresh.flow("early"))._tag).toBe("Registered")
        expect(registrations.events).toEqual([
          "register:early#1",
          "register:early#2",
      ...
      const registrations = runtime({
        onRegister: (tag, count) =>
          Effect.gen(function*() {
            ...
            // Only the first rebuild waits, so a second one that is not held
            // off by the gate has a whole second to overlap it.
            if (count === 2) yield* Deferred.await(hold)
            depth -= 1
