---
title: "API reference"
description: "Every public export of @smthrs/integrations: signatures, behavior, and errors, for the Core, GitHub, Linear, Telegram, Slack, Google Calendar, Gmail, and X namespaces."
---

The package exports eight namespaces from the aggregate entry point
`@smthrs/integrations` and from the per-provider subpaths
`@smthrs/integrations/core`, `/github`, `/linear`, `/telegram`, `/slack`,
`/googlecalendar`, `/gmail`, and `/x`. The
...
| `layer`             | `(config?: LinearConfig, env?) => Layer<LinearClient, IntegrationError>`             | An invalid config fails the build with `invalid-config`.                                                                         |
...
fields: `maxTimestampSkewMs`, `name`, `credential`, `secret`, `route`, and
`project`. A channel always verifies against the live clock, so it takes no
`nowMs`.
...
| `layer`              | `(config?: Partial<TelegramConfig>, env?) => Layer<TelegramClient, SmithersError>` | A missing token or invalid config fails the build with `INVALID_INPUT`.                                                                                                                                                                                                                                                                             |
...
| Method | Signature                                                                                                                               | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `poll` | `(cursor: string \| null) => Effect<Batch, IntegrationError>`                                                                           | One poll turn against the stored offset. Commits nothing. A stored cursor that is not a decimal offset fails `invalid-config` rather than replaying the backlog. A failed `getUpdates` fails `poll-failed`, with `details.retryable` true for a transient failure. A result that is not an update array, an update with no numeric `update_id`, or an update whose `message`, `edited_message`, or `callback_query` is not an object fails `decode-failed`. |
...
The default `run` schedule polls forever with 250 milliseconds between turns. A transient `getUpdates` failure (a network error, a timeout, an exhausted 429, or a Bot API 5xx) is logged and retried with exponential backoff from 250 milliseconds up to 30 seconds, so it never ends `run`. A permanent failure such as a 401, an unusable cursor, or an undecodable batch still fails `run`.
...
on the presser's user id, which `decision` does. The per-approval token is a 32-bit namespace, not a
secret, and it is what keeps one prompt's buttons from resolving another's:
a press whose token does not match is ignored, and a prompt built with no
...
| `decision`                | `(callbackQuery, spec: KeyboardSpec, nowMs?: number) => Outcome`                             | `Decided` with a `Decision` (approve mode) or `Selection` (select mode) only for an authorized press of an option this prompt offered. Every other press is `Ignored` with a `reason` and leaves the approval pending. `decidedAt` is the resolution wall clock; Telegram does not report when a button was pressed. |
...
(`approved`, `note`, `decidedBy`, `decidedAt`), `Selection` (`selected`,
`notes`), and `Outcome` (`{ _tag: "Decided", decision }` or
`{ _tag: "Ignored", reason }`, where `reason` is `foreign-prompt`,
`unauthorized`, or `unknown-option`).

`decision` returns `Decided` only when `from.id` is in
`spec.allowedChatIds`, compared as strings; an unlisted sender is
`Ignored` with reason `unauthorized`, never a rejection. Pass the source's allowlist here
...
The Slack surface, exported as `Slack` or from `@smthrs/integrations/slack`.
The [Slack guide](./guides/slack.md) shows a host end to end.

### Slack.Config

`resolve(config?, env?)` applies `SMITHERS_SLACK_BOT_TOKEN`,
`SMITHERS_SLACK_APP_TOKEN`, `SMITHERS_SLACK_SIGNING_SECRET`, and
`SMITHERS_SLACK_API_BASE_URL`, validates every limit (`maxRateLimitRetries`,
`maxRetryAfterSeconds`, `maxRetries`, `retryBaseDelay`, `requestTimeout`), and
returns each secret as `Redacted`. A secret may be supplied as a string or
already `Redacted`. `policy(env?)` builds a `Payload.Policy` from
`SMITHERS_SLACK_TEAM_IDS`, `SMITHERS_SLACK_CHANNEL_IDS`,
`SMITHERS_SLACK_USER_IDS`, and `SMITHERS_SLACK_SELF_USER_IDS` (comma-separated)
and throws `invalid-config` for one that admits nothing. `ENV` names every
variable.

### Slack.SlackClient

The Web API client: `call(method, params?, { auth?: "bot" | "app", write? })`
and `paginate(method, itemsKey, params?, { maxPages?, limit? })`. A refusal
(`ok: false`) is typed with `details.slackError`; auth and scope refusals are
`permission-denied`. Rate limits (429 and `ratelimited`) retry for every
method. A 5xx, a server error code, a dropped connection, or a timeout on a
write (`chat.*`, `reactions.*`, `conversations.join`, `conversations.open`)
fails `outcomeUnknown` and is never repeated; on a read it retries with
backoff. `make(config?, env?, tokens?)` and `layer`; `tokens` supplies
`AccessTokenSource`s that take precedence and are treated as rotating.

### Slack.Payload

The schemas (`EventCallback`, `MessageEvent`, `BlockActions`,
`SocketEnvelope`, `ChannelId`, `Ts`, …) and the admission every door shares.

| Export                  | Signature                                                        | Notes                                                                                                                                                                                              |
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Policy`                | interface                                                        | `allowedTeamIds` (required), `allowedChannelIds`, `allowedUserIds`, `selfUserIds`.                                                                                                                 |
| `requirePolicy`         | `(policy, where) => Policy`                                      | Throws `invalid-config` without a workspace, or without a conversation and a person.                                                                                                               |
| `classify`              | `(payload, policy) => Classification`                            | `Admitted` with the delivery key and names, or `Refused` with `unsupported`, `team-not-allowed`, `channel-not-allowed`, `bot-author`, `self-author`, or `user-not-allowed`, checked in that order. |
| `isDirectMessage`       | `(channelId) => boolean`                                         | A `D…` conversation. Admitted without listing it when `allowedUserIds` is set.                                                                                                                     |
| `toExternalEvent`       | `(payload, { policy, source?, receivedAtMs? }) => ExternalEvent` | Names the event at its most specific form, correlates it to its thread, removes `token` and `response_url`. Throws `SlackRefused` for a refused delivery.                                          |
| `idempotencyKey`        | `(payload) => string \| undefined`                               | `slack:<team>:<event id>` or `slack:<team>:action:<trigger id>`.                                                                                                                                   |
| `correlations`, `names` | `(payload) => …`                                                 | The thread, channel, `null` ladder, and the event names, most specific first.                                                                                                                      |

When `allowedUserIds` is set, every admitted delivery's author (a button's
presser) must be one of those people.

### Slack.SocketSource

`make(options, env?)` builds a Socket Mode source; `run(onBatch)` connects
with the app token, waits for `hello`, and delivers each admitted
`events_api` or `interactive` envelope as a one-event batch. The envelope is
acknowledged only after `onBatch` succeeds; a failing `onBatch` fails `run`
unacknowledged. Refused, unsupported, and oversized envelopes are
acknowledged and dropped. A redelivered event handled in this process is
acknowledged and not delivered again. `disconnect` reconnects at once; a
dropped connection after `reconnect.initialDelay`; `link_disabled` fails
`permission-denied`; `reconnect.maxAttempts` consecutive failed connections
fail `poll-failed`. Options: `policy` (required), `sourceId`, `client`,
`reconnect`, `helloTimeout`, `dedupeCapacity`, `maxEnvelopeBytes`,
`allowPlaintextSocket`, and the `SlackConfig` fields. `idempotencyKey(event)`
is the key to pass on to `Control.signal`.

### Slack.Connections

| Export                 | Signature                                                                    | Notes                                                                                                                                                         |
| ---------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SlackConnections`     | service                                                                      | `resolve(connectionId, channel)`: the client, once the channel is in the connection's containers (`*` allows all). Otherwise `permission-denied`, no request. |
| `make`, `layer`        | `(entries: Resolved[]) => …`                                                 | Refuse a non-Slack or duplicated connection with `invalid-config`.                                                                                            |
| `fromEnvironment`      | `({ containers, connectionId?, scopes?, ...limits }, env?) => Resolved`      | Connection `slack` (by default) from `SMITHERS_SLACK_BOT_TOKEN` (required, else `credentials-missing`) and `SMITHERS_SLACK_APP_TOKEN`.                        |
| `layerFromEnvironment` | same `=> Layer<SlackConnections, IntegrationError>`                          | The resolver over that one connection.                                                                                                                        |
| `fromConnection`       | `({ connection, credentials, access, appCredential?, limits? }) => Resolved` | Tokens resolved per call through `Core.Connection.resolveSecret`, host policy first.                                                                          |
| `clientFor`            | `(connection, tokens, limits?) => SlackClient`                               | A client pinned to the connection's API origin.                                                                                                               |
...
Block Kit button approvals. `token(id)` namespaces a prompt; `blocks(spec)`
builds the `actions` block (`approve` mode: approve and reject buttons;
`select` mode: one per option). `decision(payload, spec, nowMs?)` answers
`Decided` only for a press by a user in `spec.allowedUserIds` (and a workspace
in `spec.allowedTeamIds` when given) of an option this prompt offered; every
other press is `Ignored` with `foreign-prompt`, `unauthorized`, or
`unknown-option`, and the approval stays pending. `pressedToken(payload)`
names the prompt a press belongs to. `actionId` and `parseActionId` encode
`sap:<token>:a|d|s:<key>`.

### Slack.Webhook

The Events API door: `verify` (the `v0` HMAC over the exact bytes, with a
timestamp skew bound), `challenge`, `answer` (for a host that owns the HTTP
response), `decode`, `idempotencyKey`, and `channel(options)`, a
`@smthrs/control` channel that verifies, decodes, and routes one event.

### Slack.Sync

`make(options)` is a `Core.Sync` adapter over one conversation's history and
recent threads (`connectionId`, `channel`, `client`, optional `channelType`,
`pageSize`, `initialOldest`, `maxTrackedThreads`, `threadWindowSeconds`,
`maxReplyPages`, `workspaceUrl`). `messageRecord` and `eventRecord` map
messages and `message_changed`/`message_deleted` events to source records.

## Google Calendar

Exported as `GoogleCalendar` or from `@smthrs/integrations/googlecalendar`.
See the [Google Calendar guide](./guides/google-calendar.md).

- `Config`: `resolve` over `SMITHERS_GOOGLE_*`; `SCOPES`.
- `CalendarClient`: events list/get/insert/patch/delete/instances, free/busy,
  token refresh on 401, rate-limit waits. `make`, `layer`,
  `layerFromConnection(connection, access, config?, env?)`.
- `Event`, `EventId`: event schemas and input validation; `EventId.fromKey`
  derives the deterministic base32hex id an upsert inserts under.
- `Actions`: `UpsertEvent`, `PatchEvent`, `CancelEvent`, `FreeBusy`, each with
  an `idempotencyKey`, `implementationVersion`, and the bounded
  `retryPolicy`; `EventConflict` when an upsert's id holds a different event.
- `Sync`: `make` builds the `syncToken` change feed; `render` and `toRecord`
  map events to records.

## Gmail

Exported as `Gmail` or from `@smthrs/integrations/gmail`. See the
[Gmail guide](./guides/gmail.md).

- `Config`: `resolve`; the token is an `AccessTokenSource`, never ambient.
...
- `Actions`: `CreateDraft`, `SendMessage` (irreversible, `outcomeUnknown` on a
  lost answer), `FindByKey`.
- `Reconcile`: `findByKey` over the mailbox.
- `Records`, `Sync`: `mailbox` and `search` over messages and history as
  private source records.

## X

Exported as `X` or from `@smthrs/integrations/x`. **Read-only**: see the
[X guide](./guides/x.md).
