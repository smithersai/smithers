...
A terminal control status waits for that marker. Every `completed` or `failed` that `AgentSession.settle` or `settleDriverFailure` writes is held until this run's copied `flows.engine.run-decision` and `projection-settled` are in the control journal, including when the run's body finishes before its observation is registered, because a launch registers the hold before the executor runs and the observation adopts it. The hold is bounded by a 30 second grace; a wait that gives up records what it waited for and the status is written anyway. One terminal write stays unordered: `ControlLive.reconcileTerminal`, which `Control.cancel` reaches only on a run another process already completed without settling the control row. Ordering it needs a `ControlExecutor` port, because the supervisor is internal to `@smthrs/cli` while `ControlLive` lives in `@smthrs/control` and can reach no observation from there.
