...
A terminal control status waits for that marker. Every `completed` or `failed` that `AgentSession.settle` or `settleDriverFailure` writes is held until this run's copied `flows.engine.run-decision` and `projection-settled` are in the control journal, for as long as this host holds the run: a launch and an executor-port resume register the hold before the executor runs, so a body that finishes before its observation is registered is held too, and the observation adopts that hold. The hold is bounded by a 30 second grace; a wait that gives up records what it waited for and the status is written anyway. A run this host is not observing is not held at all.

Two terminal writes stay outside it. `ControlLive.reconcileTerminal` is unordered: `Control.cancel` reaches it on a run another process already completed without settling the control row, and ordering it needs a `ControlExecutor` port, because the supervisor is internal to `@smthrs/cli` while `ControlLive` lives in `@smthrs/control` and can reach no observation from there. A re-drive taken up by AgentSession's own resume bridge, which is how `Control.resume` reaches a host, registers no hold of its own, so a run resumed before this host's recovery has admitted it is not held either.
